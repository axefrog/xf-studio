...
| **Preview** | Head, eye plate and eyes derived only from the player's own game files (both hosts prepare them with a progress card); deforming head and plate with the real default creator idle (pause and head/facial subsets), saved-V import (five facial morphs, eye shape), the V's own skin, face decals (vanilla and CCXL makeup, scars, tattoos, face cyberware; one decal family with the engine's square-root-space blend) and eyes, saved brows, lashes and hair shaded with the game's decoded hair-shader colour model and light (Karis-style lobes with the game's vanilla hair options from a third-party list; profile colour space still a hypothesis, awaiting a controlled capture), the V's own piercings and the creator's graphic eye designs through a layered (`multilayered.mt`) adapter from the resolver (modular jewellery frameworks included, with a viewport-only style and colour to try), the V's own body on the idle rig with its own shapes, a visibility toggle and a whole-body view ([body rendering](../knowledge/body-rendering.md)), the clothes the save says she wears with a Clothing control (vanilla items; [worn clothing](../knowledge/clothing.md#6-how-the-studio-draws-worn-clothing)), preview quality presets and experimental finish looks. Since 26–27 September: V's **body** (resolved like the game, sharing the head's skin, censorship underwear that fails closed), **worn clothing** from the save (phases 1–4, a Clothing control), **more lifelike eyes** (eye ranks 4–5: relief-lit iris, refracted iris, the eye's own roughness), and the resolver reads game files with XF Studio's own **native reader first** (WolvenKit only as fallback; about 4 s instead of 90 s cold on the reference save). |
...
| **Desktop** | Electrobun app with a tag-driven CI release pipeline (draft pre-releases, checksums, build-provenance attestation, packaged-content privacy scan), a hand-written changelog and a unified XF icon. Releases ship one self-contained setup program (`XFStudio-<version>-win-x64-setup.exe`, an Inno Setup wrapper around Electrobun's unmodified setup), with nothing to extract. `v0.1.0-alpha.1` is tagged at `60a60e9`; its draft release asset passed an unattended clean-machine Windows Sandbox run (one-click WebView2 install through uninstall), with checksums and attestation verified. First run leads a community user from the game folder (Steam, GOG, Epic or MO2; an Xbox copy is recognised and explained) through a consented, hash-pinned WolvenKit download to a prepared 3D head. Unsigned alphas; updater disabled. See the [desktop README](../projects/xf-studio/authoring/desktop/README.md) |
| **Diagnostics** | One structured, bounded, redacted log per host and an always-on rolling window of resolver and preparation decisions; error notices carry a reference and **Report this problem**; **Report a problem** prepares a reviewable report file (mod identities and resource extracts, never mod files by default) that the person saves and attaches. Nothing is sent automatically. See [diagnostics](diagnostics.md). Browser-checked on localhost; not yet tried in the desktop app. |
...
| **Checks** | 1,817 Studio tests pass (opt-in real-install tests skipped), 88 desktop tests pass; both typechecks, the browser build, site verify, the Markdown link check and the private-data check (with its self-test, including the redacted-name rule) pass, locally and in CI (27 September). Code-health reviews run after each merge wave; open findings are in the [code-health ledger](../research/authoring/code-health.md). |
...
- **Close-up rendering:** confirmed in game on 26 September ([experiment 020](../experiments/020-session-2/README.md)): the 0 mm plate breaks up at extreme close range, 0.1 mm and 0.4 mm hold, so Build keeps the 0.4 mm lift. Eyelid-contact gates from experiments 006/012 are still open.
- **Finish looks:** Matte and Satin read glossy and Shimmer reads as soft gloss (found wrong); the gloss calibration and stronger Shimmer variants await session 2. The plate uses only about 4 % of each preset texture, which limits every finish; a plate-local UV window is now built for flat and faceted routes ([Glitter in game](../knowledge/glitter-in-game.md)). Offline since: Matte roughness is 1.0 (was 0.88) and Colour-shifting metalness 0.08 (was 0.25), from the decal shader study; the Gloss A–D, Shimmer and Metal verdicts still wait for the next session (the staged session 2 build keeps the old values as the reference).
...
- **Runtime bridge:** works in game (26 September): script calls with a proper context (the earlier TweakDB crash is fixed and checked in game), reads, writes with undo, the kill switch, character-creator presets, photo-mode camera, light, expression and captures. Since then, built and tested offline for the next session: opening photo mode with the player's own key, automatic framing, cursor hiding, light on/off, creator Confirm and Back (test profile only), burst captures, XF camera presets (test builds only), and two read-only expression probes.
...
Every pending in-game ask below and on the knowledge pages is ranked into ten sittings of 20–30 minutes, driven through the bridge, in the [next-sessions plan](../research/runtime/next-sessions-plan.md).
