...
XF Studio's eye-makeup editor works as a local prototype and can build verified private mod candidates. **Nothing has been seen in the game yet.** The first in-game smoke test is prepared and waiting on Nathan. The long-requested brow, lash and hair colour fidelity is traced offline but unfinished. Two research directives, the game shader/material system and the character-customisation file chain, were never properly started and now form the core of the R&D lab.
...
| **UI** | New dock/panel workspace (`studio-main.ts`), delivered 24 September and built by Claude under the previous coordinator. Its three audit defects are fixed. Nathan's cursory review is positive; his in-depth review is pending. The legacy shell remains at `/legacy.html` until he accepts the new UI. |
