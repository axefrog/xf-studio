...
| **Checks** | 875 Studio tests pass with 5 opt-in real-install tests skipped, 67 desktop tests pass; both typechecks, the browser build, site verify, the Markdown link check and the private-data check (with its self-test) pass, locally and in CI (25 September). Deep reviews at `19bf84c`, `524a575`, `ecb4b33` and `ac251d8` plus a runtime-bridge security review; every High they found is fixed except the two long-standing design items (UI-02 partly, CORE-03). See the [code-health ledger](../research/authoring/code-health.md) |
