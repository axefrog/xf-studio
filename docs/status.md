...
| **Checks** | 764 Studio tests pass with 5 opt-in real-install tests skipped, 60 desktop tests pass, both typechecks, the browser build, site verify and the Markdown link check pass (25 September, `7e02636`). Deep review at `19bf84c` covered the domain core and the pipeline/hosts; its one High (CORE-16) and most core findings are fixed, pipeline fixes are in progress, and the presentation/desktop review is running as the pre-alpha gate. See the [code-health ledger](../research/authoring/code-health.md). |
