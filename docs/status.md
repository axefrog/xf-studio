...
| **Checks** | 1,141 Studio tests pass with 5 opt-in real-install tests skipped, 72 desktop tests pass; both typechecks, the browser build, site verify, the Markdown link check and the private-data check (with its self-test) pass, locally and in CI (26 September). Open High findings: DIAG-05 (a problem report treats the whole game folder as one mod; fix in progress) and NATIVE-01..05 (the native archive reader, not yet wired in, could hang or exhaust memory on a hostile archive). claude/native-hardening fixes all of NATIVE-01..17; once it merges, one High is open and feature merges can resume (they pause while more than three are open). The long-standing design item CORE-03 is also open. See the [code-health ledger](../research/authoring/code-health.md) |
