...
| **Checks** | 1,141 Studio tests pass with 5 opt-in real-install tests skipped, 72 desktop tests pass; both typechecks, the browser build, site verify, the Markdown link check and the private-data check (with its self-test) pass, locally and in CI (26 September). The deep review at `972ee62` found five High findings in the new diagnostics reports (privacy and correctness: unticked parts still shipped, a space in a profile name defeats redaction, a log write can throw, empty resource sections, the game folder treated as one mod), so feature merges are paused until at most three remain; fixes run in claude/cleanup-diagnostics. The long-standing design item CORE-03 is also open. See the [code-health ledger](../research/authoring/code-health.md) |
