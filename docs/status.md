...
| **UI** | Dock/panel workspace (`studio-ui/`, started by the one composition root `studio-startup.ts` on localhost and desktop), delivered 24 September and built by Claude under the previous coordinator. Its three audit defects are fixed. The maintainer's cursory review is positive; an in-depth review is pending. The legacy sidebar shell was retired on 25 September (no legacy code paths in an alpha; older saved data still loads). |
