...
The game blinks V with short additive clips from `generic_facial_additives.anims` (fast, half, normal, slow and tiny blinks), not with a separate eyelid system. We solve its **normal** blink with the same offline solver as the idle, so the lids turn about the eye the way the game's facial setup turns them, with the lashes, brows and your makeup following. In the viewport's Motion panel, **Closure** scrubs the closing half of that blink and **Play blink** plays it at the game's speed, repeated every 2.45 seconds (a Studio choice: the average spacing of the idle's own blinks). Like the idle, it is prepared once from your own game files and never shipped; until it is, the Blink controls stay off and say so. The details, including which eye shapes still show a slit when closed, are in the [facial animation knowledge page](../knowledge/facial-animation.md).

To prepare it on a development checkout, extract `generic_facial_additives.anims`, the female head morph targets and (for comparison) the male player facial setup with WolvenKit, export the blink clips with `anim-export` using the female head rig, then run `bake_game_blink.py`; [the game's blink](../research/animation/game-blink.md) has the exact steps.
