...
Current control verification: 46 automated tests pass. The [real-asset check](../projects/xf-appearance-studio/authoring/evidence/idle-controls-offline-check.json) samples 222 phases for each of neutral and saved-face customization: structural head drift stays at numerical roundoff while gaze/jaw/lids vary, and pause/reset errors are zero. [Browser checks](../projects/xf-appearance-studio/authoring/evidence/idle-controls-browser-check.json) verify paused reload, framing, independent subsets and editing. These establish preview behavior, not live-game graph parity.
