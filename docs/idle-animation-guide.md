...
| Idea | Assessment |
|---|---|
| Play, pause, scrub and adjust the speed of an existing idle | Straightforward playback work once the relevant motion is decoded. |
...
Enable **Character-creator idle** in the viewport to preview the combined clips. Disable it to restore the editing pose; the separate exploratory blink controls are disabled while the idle plays. Expression authoring and custom idle authoring follow the feature discussion we have reserved before full-body tattoos. A promising future editor would preserve the meaningful facial controls rather than exposing hundreds of unrelated bone sliders.

Evidence: [local animation intake](../projects/xf-appearance-studio/authoring/evidence/idle-intake.json), [facial bake](../projects/xf-appearance-studio/authoring/evidence/idle-face-bake.json), [offline playback checks](../projects/xf-appearance-studio/authoring/evidence/idle-offline-check.json), [clip exporter](../projects/xf-appearance-studio/tools/anim-export/Program.cs), [playback adapter](../projects/xf-appearance-studio/authoring/src/idle-animation.ts). Extracted game animation files remain local and are not included in GitHub.
