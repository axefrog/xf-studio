...
- `D:/Dev/Cyberpunk-Modding-Docs` tracks Nathan's fork and is current to that origin, not certified current to canonical upstream. Important guidance was also checked live against the official wiki/author source.
...
- WolvenKit CLI 8.17.4: `F:/Games/RedModding/WolvenKit.Console/WolvenKit.CLI.exe`. Verified archive extraction and JSON/CR2W material round-trip. Current official WolvenKit release is [9.0.1](https://github.com/WolvenKit/WolvenKit/releases/tag/9.0.1); assess its release assets/CLI before moving production builds, then rerun the serialization fixture. GUI and CLI versions are separate tools, not interchangeable assumptions.
