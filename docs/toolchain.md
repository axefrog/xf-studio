...
| [Capstone](https://www.capstone-engine.org) Python package | 5.0.9 (PyPI wheel `capstone-5.0.9-py3-none-win_amd64.whl`) | `research/materials/shader-system/exe_hair.py`: reading hair lighting options and the hair-profile bake in the game executable | `D:/Dev/tools/README.md` (wheel SHA-256); unpacked under `<XF_TOOLS_DIR>/capstone/5.0.9/site` |
...
| [MCP TypeScript SDK](https://github.com/modelcontextprotocol/typescript-sdk) (`@modelcontextprotocol/sdk`, MIT) | 1.30.1 | The bridge's stdio MCP server (`tools/mcp/server.ts`) and the SDK client in its end-to-end tests | `projects/xf-runtime-bridge/package.json` and `bun.lock`; `bun install` in that folder |
| TypeScript and `@types/bun` for the bridge tools | 7.0.2 / 1.4.2 | `bunx tsc --noEmit` over `projects/xf-runtime-bridge/tools` | `projects/xf-runtime-bridge/package.json` |
...
| [Vortex](https://github.com/Nexus-Mods/Vortex) and its [Cyberpunk 2077 extension](https://github.com/E1337Kat/cyberpunk2077_ext_redux) | 2.7.1 and 0.12.1 | Research only: how Vortex deploys Cyberpunk mods ([Vortex knowledge](../knowledge/vortex.md)). Never installed on the host; run only in Windows Sandbox by [experiment 023](../experiments/023-vortex-sandbox/README.md). XF Studio reads Vortex's files but never depends on or starts it | `D:/Dev/tools/README.md` (official GitHub release assets, published SHA-256 verified); source clones in `D:/Dev/clones/Vortex` (tag `v2.7.1`, `c8ea03d0`) and `D:/Dev/clones/cyberpunk2077_ext_redux` (`565deef1`) |
