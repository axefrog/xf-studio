...
- The [XF Runtime Bridge](../projects/xf-runtime-bridge/README.md) is the base for such diagnostics: RED4ext, redscript, CET and TweakXL layers logging into one correlated plugin log, and a local read-only-by-default pipe for live queries. It is built and self-tested offline; its first game session is the [bridge test card](../research/runtime/runtime-bridge-test-card.md), and the APIs it may use are cited in the [capability matrix](../research/runtime/runtime-bridge-design.md#7-autonomy-capability-matrix).
...
**Current staged state (25 September, after reorganisation):** no game session has happened yet.

- **Test package:** the reference MO2 installation has a single **XF Eye Artistry** mod holding the [session 2](../experiments/020-session-2/README.md) candidate `xfs_c0200a5e52e554c029d0b0000000000d0` (Off plus 12 presets: plate-lift depth variants, gloss calibration, strong Shimmer, lifted metal ramp and the UV-window placement pair), built through the production pipeline with the built-in plate derived from the installed 2.31 game. Earlier candidates (the five-look package, the finish board and experiment 017) are kept in the ignored `local/mo2-backups/`. The mod is enabled only in the test profile `XF Studio diagnostic 2026-09-25`, where the legacy XF Eye Artistry CCXL development mod is disabled. It is disabled in the everyday profile.
- **Frameworks:** following the rule against duplicating users' frameworks, the earlier side-by-side "XF Studio Framework" copies were removed. The maintainer's own ArchiveXL, TweakXL and Codeware entries were updated in place to 1.27.3, 1.11.4 and 1.20.5 (Nexus metadata kept), so both profiles use the same framework installs.
- **Earlier promotion:** the 25 September promotion receipt no longer describes the MO2 tree, so its rollback no longer applies. A full pre-change backup (both modlists, `ModOrganizer.ini`, the framework folders and the old test mod) is kept in the ignored `local/mo2-backups/2026-09-25-reorg/`.
- **Session plan:** a short maintainer checklist covers selector presence, Off/look switching and clearing, finish appearance, eyelid contact, brow/lash/hair captures for colour calibration, and save persistence. The coordinator takes log captures before and after the session with `tools/capture_session.py --profile`.
...
2. In the character creator, check the single eye-makeup selector. The promoted candidate predates the XF Eye Artistry name, so its selector is labelled `XF Studio · Eye makeup`; candidates built since then are labelled `XF Eye Artistry`. Check **Off** and each authored verification preset in the staged candidate (for the promoted four-preset fixture from `editor-collection.json`: Metallic copy, Reload persistence, Collection B and Collection A). Switch A → B → Off, then B → A → Off; check that a choice replaces the complete prior look, that Off clears it, and that reselecting a look does not retain hidden prior components. The package manifest pins preset IDs, revisions and generated appearance names, **not display names**; confirm the ID-to-name mapping against the exact source collection snapshot.
...
Reference for the tools that prepared the current diagnostic profile. None of them launches MO2 or the game, switches MO2's selected profile, or proves runtime behaviour. None of them installs, replaces, disables or duplicates a framework or moves another mod entry: they report framework versions and add only the XF Eye Artistry entry ([framework check and placement rule](../research/authoring/framework-version-check.md)). Keep their JSON plans, receipts and journals private and ignored; they include local paths and hashes.

### Framework check

The read-only [framework check](../projects/xf-studio/authoring/tools/framework-check.ts) reports the ArchiveXL, TweakXL, Codeware, RED4ext, redscript and CET versions the game would load from the game folder and from each named MO2 profile. It compares them with the XF Eye Artistry minimums (ArchiveXL 1.27.3, RED4ext 1.29.0, redscript 0.5.31) and prints plain update guidance. For each profile it also prints where the placement rule puts XF Eye Artistry, both as listed now and as if the entry were absent. It writes nothing. The same check backs the `detect.frameworkVersions` host action and the advisory `frameworks` readiness entry.

```powershell
bun tools/framework-check.ts --game-root $gameRoot --mo2-root $mo2Root --profile $profileId
```

### Runtime diagnostic planner

The read-only [runtime diagnostic planner](../projects/xf-studio/authoring/tools/runtime-diagnostic.ts) checks a selected candidate's exact archive/XL hashes, the selected MO2 `modlist.txt`, the profile's framework versions (with update guidance among its cautions) and exact archive filenames in enabled mods and the direct game folder. Dry-run is the default. `--stage` is a separate explicit action that copies selected profile metadata (including `plugins.txt` and `loadorder.txt` when present) into a **new private scratch MO2 root**, changes only that copied modlist: it adds (or enables) the one XF Eye Artistry row at the position the [placement rule](../research/authoring/framework-version-check.md#mo2-placement-rule) chooses, and disables the older `XF Eye Artistry CCXL - Dev` development entry if enabled, and uses the [trusted paired-file transport](../projects/xf-studio/authoring/src/mod-install-transport.ts) to install only into the scratch root. The scratch root contains no copies of the other enabled mods, so it is a reviewable staging artifact, **not a launchable MO2 instance**. The planner does not compare archive members inside other mods, resolve ArchiveXL merging or prove runtime winners.
...
- **25 September promotion:** after fixture and source checks, the candidate was promoted into the new `XF Studio diagnostic 2026-09-25` profile and dedicated `XF Studio` mod (the mod's name before it became XF Eye Artistry). The source `2025 (again)` modlist hash stayed unchanged, the promoted archive pair hashes matched the manifest, and the completion receipt was present without a pending journal. Neither MO2 nor the game was launched. The same run also added side-by-side framework copies to that profile; those were later removed and the original entries updated in place (see the current staged state above).
- **Installed versions (25 September, after reorganisation):** the framework check reads ArchiveXL 1.27.3, TweakXL 1.11.4 and Codeware 1.20.5 (DLL version resources, matching MO2 metadata), redscript 0.5.31 (MO2 metadata) and RED4ext 1.30.0 and CET 1.37.1 (game folder) in both `2025 (again)` and the diagnostic profile, meeting every XF Eye Artistry minimum. The latest inspected 16 September RED4ext log (game 2.31, ArchiveXL 1.26.3, TweakXL 1.11.3, Codeware 1.20.3) describes a **past** session, not the next launch. See [toolchain](toolchain.md#runtime-evidence-and-current-stable-releases).
