...
| Location | Purpose |
|---|---|
| [XF Studio](../projects/xf-appearance-studio/README.md) | Active standalone product and project-specific source, intake, build and verification |
...
## Deferred naming and path follow-ups

The public repository is [axefrog/xf-studio](https://github.com/axefrog/xf-studio), and its display name is XF Studio. The following names remain deliberately unchanged. Review any future migration against the [naming contract](../projects/xf-appearance-studio/data/naming.md), current consumers and historical evidence before editing them.
...
| `projects/xf-appearance-studio` and depot root `axefrog/appearance_studio/` | Running application, intake and package pipelines depend on these technical paths. They are not the public product name. |
| `XFAS_DATA_DIR`, existing browser keys and serialized `eye-artistry/*` / `xfas/*` IDs | Compatibility with saved drafts, collections, SQLite revisions and older inputs; do not bulk-rename them. New recipe versions and generated game appearances follow their explicit current contracts. |
...
Newly generated archive appearances use lowercase `xfs_`. A future checkout or technical-path migration should have a consumer inventory, data migration plan, independent verification and a checkpoint of old/new paths. It is separate from this public-facing rebrand.
