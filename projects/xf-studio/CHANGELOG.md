...
- **Eyes now stay in place when you change eye shape.** In the game the eyeballs have their own copy of each of the 21 eye shapes, separate from the head's. The preview used to change only the head, so every eye shape except the starting one left the eyeballs up to about 4 mm out of their sockets. The preview built from your game files now moves the eyes with the eyelids, including while the idle animation plays and blinks. The eye-shape list now comes from the head itself and is numbered like the character creator (01 is the base shape, so the old "Eye shape 09" is now "Eye shape 10"). Checked offline and in the app's own preview on one PC with game version 2.31; not yet compared with the game side by side. Developer-prepared preview files have no eye-shape data, so their eyes still stay put, and the Character panel says so.
