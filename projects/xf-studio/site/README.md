...
- **Privacy.** The generator refuses a page, or a `knowledge/README.md` whose topic summaries the index publishes, containing a user-profile path (`C:\Users\<name>` in any spelling, WSL `/mnt/c/Users/<name>`, `/home/<name>`, `/Users/<name>`) or an e-mail address (`tools/privacy.ts`). The check applies the same guard to every published file, both as written and with its tags removed, because the renderer breaks code spans with `<wbr>`. Findings are redacted, because build logs are public. The patterns and test vectors are the repository's own (`tools/private-data.json`), shared with `tools/check_private_paths.py` and the packaged-app content scan.
