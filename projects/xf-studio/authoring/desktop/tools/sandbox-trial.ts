...
//
//   bun tools/sandbox-trial.ts [setup.zip] [--install-webview2 | --host-webview2] [--keep-open]
...
// --install-webview2 turns networking on and runs Microsoft's signed Evergreen bootstrapper
...
writeFileSync(resolve(root, "XFStudio-first-run.wsb"), `<Configuration>
  <Networking>${installWebView2 ? "Enable" : "Disable"}</Networking>
  <VGpu>Enable</VGpu>
...
console.log(`WebView2: ${installWebView2 ? "installed from Microsoft inside the sandbox (networking on)" : hostWebView2 ? `host runtime ${hostWebView2} mapped read-only` : "none (the sandbox default)"}; ${keepOpen ? "stays open" : "shuts down when done"}.`);
