...
import { canarySetupExe } from "../release";
...
// single setup program read-only and one writable results folder; networking is off, so the first run
// also proves the app starts offline. Nothing is installed on the host.
//
//   bun tools/sandbox-trial.ts [setup.exe] [--network | --install-webview2 | --webview2-installer=<file> | --host-webview2] [--keep-open]
...
//
// The run is unattended: the setup's Install button, automatic dismissal of the final installer window,
...
// --webview2-installer=<file> maps Microsoft's signed Evergreen Standalone Installer (downloaded
// beforehand from Microsoft) into the offline sandbox and runs it first, for a full first-run walk
// when the sandbox's own download of the runtime is too slow.
//
...
const setup = resolve(args.find(arg => !arg.startsWith("--")) ?? canarySetupExe);
const keepOpen = args.includes("--keep-open");
const installWebView2 = args.includes("--install-webview2");
const offlineWebView2 = args.find(arg => arg.startsWith("--webview2-installer="))?.slice("--webview2-installer=".length);
if (offlineWebView2 && !existsSync(offlineWebView2)) throw Error(`No WebView2 installer at ${offlineWebView2}.`);
...
if (!existsSync(setup) || !setup.toLowerCase().endsWith(".exe"))
  throw Error("Build the installer first (bun run build:canary) or pass the path of a single setup program (.exe).");
...
copyFileSync(setup, resolve(input, basename(setup)));
const digest = createHash("sha256").update(readFileSync(setup)).digest("hex");
writeFileSync(resolve(input, "SHA256SUMS.txt"), `${digest}  ${basename(setup)}\n`);
...
if (offlineWebView2) copyFileSync(offlineWebView2, resolve(input, "webview2-standalone-installer.exe"));
