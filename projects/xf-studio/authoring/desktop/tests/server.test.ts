...
test("session gates static files and narrowly typed host facts", async () => {
  ...
  // Resolved brows, lashes and hair: served only by content-addressed name from the host's own store.
  ...
  expect(await character.json()).toMatchObject({ phase: "failed", message: "Your V's own skin, brows, lashes and hair appear once your game folder and WolvenKit are set up." });
