...
The source maps improve this one browser lip view but do not establish the cause of Nathan's **game screenshot** or yield a release material. The current experiment omits the resolved teeth/mouth geometry, saved morph and idle pose, explicit game tangent basis, microdetail and skin translucency. Native UV0 geometry correspondence and map hashes do not prove current MO2 runtime winners. The next gate is a matched saved-V game capture with effective resource/log provenance and a view/lighting matrix; only then can an approximation be assessed for production. Until then, keep this opt-in study page and do not change the production renderer.
