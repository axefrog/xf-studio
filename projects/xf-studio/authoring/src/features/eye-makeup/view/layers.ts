...
import { openLayerMenu } from "./layer-menu";

export function layersPanel(ctx: EyeMakeupViewContext): PanelController {
  const view = ctx.facade.view(), { finishOf } = catalogues(ctx);
  const count = h("span", { class: "count" });
  const add = button({ label: "Add layer", icon: "plus", small: true,
    onClick: () => { if (ctx.dispatch(addLayer())) ctx.feedback.announce("Layer added at the front and selected"); } });
  ...
  const list = new ItemList<{ id: string; name: string; meta: string }>({
    ...
    onSelect: id => ctx.dispatch({ kind: "layer.select", layerId: id }),
    onMove: (id, visual) => ctx.dispatch({ kind: "layer.edit", command: { kind: "move", id, to: toVisual(visual) } }),
    onRename: (id, name) => ctx.dispatch({ kind: "layer.edit", command: { kind: "rename", id, name } }),
    onDelete: id => removeLayer(id),
    onDuplicate: id => ctx.dispatch({ kind: "layer.edit", command: { kind: "duplicate", id } }),
    onMenu: (id, anchor, invoker) => openLayerMenu(ctx, id, anchor, invoker),
  ...
  function removeLayer(id: string) {
    const name = view.recipe().layers.find(layer => layer.id === id)?.name ?? "Layer";
