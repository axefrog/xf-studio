 ...
 * (`features/<id>/view/`), and the composition roots join them (`compose/views.ts`). Each record is keyed
 * by exactly its view's panel IDs, so a contributed panel without a factory, or a factory for a panel no
 * view contributes, does not compile.
 */
 ...
 * each feature's) and the factory of each, keyed by the catalogue's IDs.
 */
export type ViewComposition = {
  readonly catalogue: ViewCatalogue;
  readonly factories: Readonly<Record<string, PanelFactory>>;
};
