  ...
  return section("components", "04", "Components",
    ...
    pattern({ id: "c-chips", title: "Chips, badges and status vocabulary", status: "implemented",
      specimen: `<div class="stack-s"><div class="row wrap gap-s">${chip("Based on r3")}${chip("Not in library", "warning")}${chip("Newer r5 saved", "warning")}${chip("Working…", "info")}</div>
        <div class="row wrap gap-s">${badge("Flat adapter · provisional", "success")}${badge("Preview study", "warning")}${badge("Current", "success")}${badge("Stale — draft changed since", "warning")}${badge("Blocked", "error")}</div></div>`,
      what: "Chips describe document/library state; badges qualify a result or capability. The words are fixed vocabulary: Working, Preview study, Current, Stale, Blocked. A finished Build states its limits in one plain sentence (built and checked, not tested in game, nothing installed) rather than in badges.",
