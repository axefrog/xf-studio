...
export function components() {
  return section("components", "04", "Components",
    ...
    pattern({ id: "c-context", title: "Context menu", status: "implemented",
      specimen: menu(`${menuHeading("Contour point", "Mirrored copy · edits the authored side")}${menuItem("Select point", { icon: "target", focus: true })}${menuItem("Remove point", { icon: "trash", danger: true, reason: "A shape needs at least three points." })}${menuItem("Smooth handles", { checked: true })}${menuItem("Corner handles", { icon: "shape" })}${menuItem("Set point pigment…", { icon: "edge" })}${menuItem("Set point edge softness…", { icon: "edge", reason: "Enable point edge softness before editing an individual edge." })}${menuSep}${menuHeading("UV view", "View changes are not edits")}${menuItem("Fit shape", { kbd: "F" })}`),
      what: "Target-aware commands for the thing under the cursor: collection, preset, layer, point, tangent, warp, shape interior, empty UV and viewport background. Headings name the target; disabled items stay focusable and show the reason in words.",
      when: "Right-click (a right-drag still pans), Shift+F10 or the Menu key; ⋯ buttons open the same menu for rows.",
      combine: "View commands follow a separator and are labelled as not being edits. Commands needing a value open a value popover.",
