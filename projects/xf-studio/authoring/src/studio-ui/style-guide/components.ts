  ...
  return section("components", "04", "Components",
    ...
    pattern({ id: "c-creator", title: "Creator options (generated rows)", status: "implemented", wide: true,
      specimen: `<div class="cc-panel stack-s" style="max-width:420px">
        ...
        <p class="note cc-legend"><span class="cc-legend-text"><span class="cc-fetch-mark" data-fetch="pending" aria-hidden="true"></span> Not prepared yet: the first time, XF Studio reads it from your game files, which takes a few seconds. <span class="cc-fetch-mark" data-fetch="fetching" aria-hidden="true"></span> Being prepared in the background.</span></p>
        <section class="cc-section"><h4 class="cc-section-title">Eyes</h4><div class="cc-rows">
          <div class="cc-row changed"><div class="cc-row-head"><button class="cc-row-main" type="button" aria-expanded="true">${i("chevronRight")}<span class="cc-row-label">Eye Color</span><span class="cc-row-current"><span class="swatch cc-row-swatch" style="--swatch:#3d6a8c"></span><span class="cc-row-value">Blue</span></span><span class="cc-row-not-shown" aria-hidden="true">${i("eyeOff")}</span></button><span class="cc-row-actions">${btn("Back to your V's own: Gradient brown", { icon: "reset", iconOnly: true, small: true, variant: "quiet" })}</span></div>
            <div class="cc-choice-list"><div class="cc-choices grid" role="listbox" aria-label="Eye Color choices">${["#5a3a22", "#3d6a8c", "#4f7d4a", "#7a6a5a", "#2a2a2a", "#8a5ab0"].map((colour, n) => { const fetch = ["", "", "fetching", "pending", "pending", "failed"][n]!; const words = { "": "", pending: "; not prepared yet", fetching: "; being prepared", failed: "; couldn't be prepared ahead; choosing it tries again" }[fetch]; return `<button class="cc-choice swatch-choice" type="button" role="option" aria-selected="${n === 1}" tabindex="${n === 1 ? 0 : -1}" aria-label="Choice ${n + 1}" aria-description="From the game${words}"${fetch ? ` data-fetch="${fetch}"` : ""}><span class="swatch" style="--swatch:${colour}"></span></button>`; }).join("")}</div></div></div>
        </div></section>
      ...
      combine: "One status line of fixed height above (never moves the layout): Updating… (for a choice that wasn't prepared ahead, why it takes a few seconds the first time), what failed, or the first of the plain lines about what couldn't be used, clamped to one line; Try again, Keep my changes and Details keep their place when not offered. Undo and Redo for creator changes sit beside the V's source; they are separate from the makeup's history.",
      drives: `${code("character.setOption")} (with a switcher choice's activated options), ${code("character.hideOwnMakeup")}, ${code("character.reset")}, ${code("character.resetAll")}, ${code("character.keepChanges")}, ${code("character.retry")}, ${code("character.undo/redo")}; ${code("authoring.characterPanel()")}, ${code("characterView()")}, ${code("characterChoices(option, want, query)")} and ${code("characterSearch(query)")} (frozen, paged), ${code("characterPrefetch(option, positions, focus)")} and ${code("characterStopPrefetch(option)")} (each choice's prepared-ahead state, polled per row), ${code("character.clearPreparedFiles")} beside the 3D view's controls, coverage notes from the preview's projection.`,
      ...
      a11y: "Rows are buttons with aria-expanded and an accessible description (where the option comes from, the V's own choice, whether the 3D view draws it). Choices are a listbox: arrow keys, Home and End move focus without choosing (each choice prepares the V anew), Enter or Space chooses; the chosen one is aria-selected by identity, and each choice's origin is its accessible description. Choosing keeps focus on the same item. A choice's prepared-ahead state joins its accessible description and tooltip (\"not prepared yet\", \"being prepared\"); the marks are decoration (aria-hidden in the legend), sit in the item's corner so they never move the layout, keep their own fill to stay legible over any swatch in both themes, and stop turning under reduced motion. Hovering or focusing a choice prepares it next. Ctrl+Z and Ctrl+Y inside the panel step the character's own history." }),
