import { badge, btn, chip, code, empty, group, i, menuHeading, menuItem, menuSep, note, pattern, row, section, segmented, slider, toast, toggle } from "./kit";
...
export function states() {
  return section("states", "05", "Communicating state",
    ...
    pattern({ id: "t-saved", title: "Saved versus unsaved", status: "implemented", wide: true,
      specimen: `<div class="stack-s"><div class="row wrap gap-s"><span class="status-item">● Draft autosaved in this browser</span>${chip("Not in library", "warning")}${chip("Based on r3")}${chip("Newer r5 saved", "warning")}${chip("Working…", "info")}</div>
    ...
    pattern({ id: "t-disabled", title: "Disabled with a reason", status: "implemented",
      specimen: `<div class="row wrap gap-s">${btn("Export layer mask", { icon: "export", small: true, disabled: true, title: "Select a layer before exporting a mask." })}<span class="note warning">Select a layer before exporting a mask.</span></div>`,
      what: "Every unavailable command explains itself with the application's reason: tooltip on buttons, visible text in menus, palette entries and under controls.",
    ...
    pattern({ id: "t-errors", title: "Errors and recovery", status: "implemented",
      specimen: toast("error", "Library", "The library has a newer revision of this collection. Your draft is kept.", ["Refresh library", "Save as copy"]),
      what: "Errors say what happened, confirm what was kept, and offer the specific recovery: conflicts offer Refresh and Save as copy; invalid imports keep the current draft; failed previews keep the last complete textures and offer Rebuild.",
...
export function compositions() {
  ...
  const packaging = `<div class="mock-shell">${mockHeader("Chrome dusk")}<div class="mock-dock wide-2">
    ...
    ${group([["Layers", "layers"]], 0, `<div class="panel-content"><ol class="item-list">${row("Glitter veil", "Glitter · 60%", { swatch: "#8c6fb0", finish: "glitter", warn: true, selected: true })}</ol><div class="export-line">${badge("Preview only", "warning")}<span class="small">Preview only in this alpha. Check and Build leave out layers with this finish and tell you which.</span></div></div>`)}</div></div>`;
