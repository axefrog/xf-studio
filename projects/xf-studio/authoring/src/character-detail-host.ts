...
const NEEDS_SETUP = "Your V's own skin, face details, eyes, brows, lashes and hair appear once your game folder and WolvenKit are set up.";
const PREPARING = "Preparing your V's skin, face details, eyes, brows, lashes and hair…";
const FAILED = "Something went wrong while preparing your V's skin, face details, eyes, brows, lashes and hair, so they aren't shown. The head still works.";
  ...
  request(request: CharacterRequest): CharacterDetailState {
    ...
    const promise = begun
      .then(result => {
        ...
        this.options.log?.(`Skin, face details, eyes, brows, lashes and hair prepared in ${((Date.now() - started) / 1000).toFixed(1)} s (${request.source} V).`);
      })
      .catch(error => {
        ...
        this.options.log?.(`Skin, face details, eyes, brows, lashes and hair were not prepared: ${error instanceof CharacterDetailError ? `${error.code} ${error.detail}` : (error as Error)?.stack ?? error}`);
