 ...
 * alone, neither the body nor its clothes (PREV-108); a v5 request draws the body.
 */
...
export const CHARACTER_REQUEST_SCHEMA = "xfs/character-request-6" as const;
const EARLIER_REQUEST_SCHEMAS: readonly string[] = ["xfs/character-request-1", "xfs/character-request-2", "xfs/character-request-3", "xfs/character-request-4",
  "xfs/character-request-5"];
/** Earlier versions that carry creator choices and parts (v4, v5). */
const CHOICE_SCHEMAS: readonly string[] = [CHARACTER_REQUEST_SCHEMA, "xfs/character-request-5", "xfs/character-request-4"];
/** Earlier versions that carry clothing (v5). */
const CLOTHING_SCHEMAS: readonly string[] = [CHARACTER_REQUEST_SCHEMA, "xfs/character-request-5"];
...
/** `body: false`: the V with its body (and so its clothes) turned off; absent, the body is drawn. */
export type CharacterRequest =
  ...
  | { schema: typeof CHARACTER_REQUEST_SCHEMA; source: "save"; bodyGender: BodyGender;
      appearances: SavedAppearance[]; morphs: SavedMorph[]; choices?: CharacterChoice[]; clothing?: CharacterClothing; body?: false };
 ...
 * carried and the request says so, so the host prepares the head alone.
 */
export function characterRequestOf(base: { bodyGender: BodyGender; saved: SavedDescriptors | null }, choices: readonly CharacterChoice[] = [],
  parts: readonly CcoPart[] = ["head", "body", "arms"], clothing: CharacterClothing | null = null, body = true): CharacterRequest {
...
export function parseCharacterRequest(value: unknown): CharacterRequest {
  ...
  // An earlier request carries neither choices nor parts (v4 carries both, and no clothing; v5 no body switch); a tried choice (v2, v3) is refused.
  const withChoices = CHOICE_SCHEMAS.includes(String(doc.schema)), withClothing = CLOTHING_SCHEMAS.includes(String(doc.schema));
  const optional = current ? ["choices", "clothing", "body"] : withClothing ? ["choices", "clothing"] : withChoices ? ["choices"] : [];
  let choices: { choices?: CharacterChoice[]; clothing?: CharacterClothing; body?: false } = {};
  if (current && doc.body !== undefined) {
...
/** The same V, whatever choices, clothing and body switch are set on it (a change of any keeps the V's other details on screen). */
export function sameCharacter(a: CharacterRequest, b: CharacterRequest): boolean {
  const { choices: _a, clothing: _c, body: _e, ...left } = a, { choices: _b, clothing: _d, body: _f, ...right } = b;
