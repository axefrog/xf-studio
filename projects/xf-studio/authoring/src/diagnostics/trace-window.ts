 ...
 *   size, are deleted. One event is at most `EVENT_BYTES`; a larger one keeps its keys and says how big it was.
 * - **Redacted as written** by the shared personal-data rules (the report redacts again, with the configured folders too).
...
import { boundJson, type DiagnosticTrace, type TraceEntry } from "./model";
...
export class TraceWindow implements DiagnosticTrace {
  ...
  event(area: string, event: string, data?: Readonly<Record<string, unknown>>): void {
    try {
      const entry: TraceEntry = { t: new Date(this.now()).toISOString(), area, event, ...(data ? { data: boundJson(data) as Record<string, unknown> } : {}) };
      let line = JSON.stringify(entry);
      if (line.length > EVENT_BYTES) line = JSON.stringify({ ...entry, data: { truncated: true, bytes: line.length, keys: Object.keys(data ?? {}).slice(0, 40) } });
