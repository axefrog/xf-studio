...
test("a head step failing after the scene loaded releases every head connection, and Try again attaches once", async () => {
  ...
  expect(h.attached).toEqual({ savedV: undefined, preview: undefined, motion: undefined });
  ...
  expect(h.attached).toEqual({ savedV: head.savedAppearance, preview: head.preview, motion: head.motion });
