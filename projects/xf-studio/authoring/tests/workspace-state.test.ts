...
test("an absent browser workspace starts with one editable four-point makeup area", () => {
  ...
  const starter = loadWorkspace(empty, false, STUDIO_DOCUMENTS);
  ...
  const restored = loadWorkspace({ getItem: key => key === keys.workspace ? JSON.stringify(serializeWorkspace(saved, STUDIO_DOCUMENTS)) : null }, false, STUDIO_DOCUMENTS);
  expect(restored.state.recipe).toEqual(saved.recipe);

  const historical = initialRecipe();
  expect(historical.layers).toHaveLength(4);
  const legacy = loadWorkspace({ getItem: key => key === keys.legacy ? JSON.stringify(historical) : null }, false, STUDIO_DOCUMENTS);
...
test("workspace migration preserves legacy drafts and isolates verification; unreadable work is protected", () => {
  ...
  expect(loadWorkspace(storage, false, STUDIO_DOCUMENTS).state.recipe).toEqual(draft);
  expect(loadWorkspace(storage, true, STUDIO_DOCUMENTS).state.recipe).not.toEqual(draft);
  expect(main.workspace).not.toBe(verify.workspace);
  records.set(main.workspace, "{broken");
  const failed = loadWorkspace(storage, false, STUDIO_DOCUMENTS);
  expect(failed.writable).toBe(false);
  expect(failed.state.recipe).toEqual(draft);
  expect(records.get(main.workspace)).toBe("{broken");
  expect(loadWorkspace({ getItem() { throw Error("denied"); } }, false, STUDIO_DOCUMENTS).writable).toBe(false);
});
...
test("workspace restores edited selection, camera, library revision and undo without aliasing", () => {
  ...
  const sanitized = parseWorkspace(storedWorkspace(state), STUDIO_DOCUMENTS);
  ...
  expect(() => parseWorkspace({ ...storedWorkspace(state), schema: "future" }, STUDIO_DOCUMENTS)).toThrow();
  expect(() => parseWorkspace({ ...storedWorkspace(state), savedV: {} }, STUDIO_DOCUMENTS)).toThrow();
});
...
test("preview texture size persists independently of recipes; older documents keep defaults", () => {
  ...
  const old = storedWorkspace(state); delete old.preview.textureSize;
  const migrated = parseWorkspace(old, STUDIO_DOCUMENTS);
  expect(migrated.preview.textureSize).toBe(1024);
  for (const size of [null,"2048",0,768,8192,NaN,Infinity]) {
    const malformed = parseWorkspace({ ...storedWorkspace(state),preview:{...state.preview,textureSize:size},panels:{previewQuality:"true"}}, STUDIO_DOCUMENTS);
...
test("paused idle and contribution choices round trip, and legacy workspaces preserve both motions", () => {
  ...
  expect(parseWorkspace(storedWorkspace(state), STUDIO_DOCUMENTS)).toEqual(state);
  ...
  const malformed = parseWorkspace({ ...storedWorkspace(state), preview: { ...state.preview, idlePaused: "true", idleBody: 0, idleFace: null } }, STUDIO_DOCUMENTS);
  ...
  const disabled = parseWorkspace(storedWorkspace(state), STUDIO_DOCUMENTS);
...
test("the 3D preview's automatic start is a workspace preference that defaults on", () => {
  ...
  const off = parseWorkspace(JSON.parse(JSON.stringify({ ...storedWorkspace(fresh), previewSetup: { autostart: false } })), STUDIO_DOCUMENTS);
