...
test("an absent browser workspace starts with one editable four-point makeup area", () => {
  ...
  const restored = loadWorkspace({ getItem: key => key === keys.workspace ? JSON.stringify(serializeWorkspace(saved)) : null }, false);
...
test("workspace restores edited selection, camera, library revision and undo without aliasing", () => {
  ...
  const sanitized = parseWorkspace(storedWorkspace(state));
  ...
  expect(() => parseWorkspace({ ...storedWorkspace(state), schema: "future" })).toThrow();
  expect(() => parseWorkspace({ ...storedWorkspace(state), savedV: {} })).toThrow();
});
...
test("far narrow-FOV framing survives reload but out-of-range orbit does not", () => {
  ...
  expect(parseWorkspace(storedWorkspace(state)).preview.camera).toEqual(state.preview.camera);
  state.preview.camera.position[2] = -4;
  expect(parseWorkspace(storedWorkspace(state)).preview.camera).toBeUndefined();
});
...
test("preview texture size persists independently of recipes; older documents keep defaults", () => {
  ...
  for (const size of [null,"2048",0,768,8192,NaN,Infinity]) {
    const malformed = parseWorkspace({ ...storedWorkspace(state),preview:{...state.preview,textureSize:size},panels:{previewQuality:"true"}});
...
test("paused idle and contribution choices round trip, and legacy workspaces preserve both motions", () => {
  ...
  expect(parseWorkspace(storedWorkspace(state))).toEqual(state);
  ...
  const malformed = parseWorkspace({ ...storedWorkspace(state), preview: { ...state.preview, idlePaused: "true", idleBody: 0, idleFace: null } });
  ...
  const disabled = parseWorkspace(storedWorkspace(state));
...
test("the 3D preview's automatic start is a workspace preference that defaults on", () => {
  ...
  const off = parseWorkspace(JSON.parse(JSON.stringify({ ...storedWorkspace(fresh), previewSetup: { autostart: false } })));
  expect(off.previewSetup).toEqual({ autostart: false });
  expect(parseWorkspace(JSON.parse(JSON.stringify({ ...storedWorkspace(fresh), previewSetup: { autostart: "no" } }))).previewSetup).toBeUndefined();
});
