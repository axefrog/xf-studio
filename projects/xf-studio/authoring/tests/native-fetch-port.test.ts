...
const fixtureModule = join(import.meta.dir, "fixtures", "native-archive.ts");
...
test("a worker decodes with a time budget: a resource over it falls back and the next one gets a fresh worker", async () => {
  ...
  const fast = new WorkerDecoder({ ...options, decompressor: { module: fixtureModule, export: "fakeDecompress" }, timeoutMs: 20_000 });
  ...
  const slow = new WorkerDecoder({ ...options, decompressor: { module: fixtureModule, export: "slowDecompress" }, timeoutMs: 400 });
