...
import { encodeWorkspacePlan, fitWorkspace, MIN_SELECTED_HISTORY, PERSISTED_BACKGROUND_HISTORY, WORKSPACE_STORAGE_BUDGET } from "../src/workspace-budget";
...
function wideRecipe(layers: number): Recipe {
  ...
  while (recipe.layers.length < layers) recipe = editLayers(recipe, recipe.layers[0].id, { kind: "duplicate", id: recipe.layers.at(-1)!.id }).recipe;
  return recipe;
}
...
test("the platform transaction: one step named by the first change, empty ones leave no trace, Escape never creates Redo", () => {
  ...
  const host: TransactionHost<number> = {
    checkpoint: () => { steps.push(content); return steps.length; },
    relabel: (step, label) => { steps[step - 1] = `${steps[step - 1]}:${label.label}`; },
    discard: step => steps.length === step ? (steps.pop(), true) : false,
    revert: () => { reverted.push(content); content = steps.pop()!.split(":")[0]; },
    content: () => content,
  };
...
test("fitWorkspace keeps the standard form when it fits and trims in order when it does not", () => {
  const state = busyWorkspace(4, 80);
  const standard = fitWorkspace(state, STUDIO_DOCUMENTS);
  expect(standard).toMatchObject({ trimmed: false, overBudget: false });
  const restore = (encoded: string) => parseWorkspace(JSON.parse(encoded), STUDIO_DOCUMENTS);
  const depthOf = (fitted: { encoded: string }) => {
    const collections = restore(fitted.encoded).collections!;
  ...
  // 1–2: a little over the standard form gives up other presets' steps first, oldest first.
  const tight = fitWorkspace(state, STUDIO_DOCUMENTS, standard.size - 1);
  expect(tight).toMatchObject({ trimmed: true, overBudget: false });
  expect(tight.plan.selected).toBe(Infinity);
  ...
  const floor = fitWorkspace(state, STUDIO_DOCUMENTS, atFloor + 2_000);
  expect(floor.plan.background).toBe(0);
  expect(depthOf(floor)[0]).toBeGreaterThanOrEqual(MIN_SELECTED_HISTORY);
  expect(depthOf(floor)[0]).toBeLessThan(80);
  expect(floor.plan.recovery).toBe(Infinity);
  ...
  expect(restore(smallest.encoded).collections!.collection.presets).toHaveLength(4);
});
...
test("a reload under budget pressure keeps at least 10 Undo steps of the selected look (gate)", () => {
  ...
  const standard = fitWorkspace(state, STUDIO_DOCUMENTS);
  // A budget that holds the looks and a little history: the older fixed levels kept none of the selected look's steps here.
  const floor = encodeWorkspacePlan(state, { background: 0, selected: MIN_SELECTED_HISTORY, recovery: 0, removed: 0 }, STUDIO_DOCUMENTS).length;
  const budget = floor + 5_000;
