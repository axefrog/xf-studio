 ...
 *
 * It also writes `src/native/rtti-class-hashes.json`: the FNV-1a 32 hash of every class name in the dump (base 36, comma-separated),
 * so the reader can tell a class the game knows but the slice leaves out from a name the RTTI does not know at all (which a
 * package refuses rather than read as a struct). `--hashes-only` writes just that file.
 *
 ...
 *
 *   bun tools/native-rtti-subset.ts --dump <red-dump-json folder> [--observed <folder of WolvenKit JSON>]... [--class <name>]... [--hashes-only]
 */
import { readdirSync, readFileSync, statSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { classNameHash } from "../src/native/rtti";
...
// Every class name, hashed: the file names of `classes/` are the class names.
const classNames = readdirSync(join(dump, "classes")).filter(name => name.endsWith(".json")).map(name => name.slice(0, -5));
const hashes = [...new Set(classNames.map(classNameHash))].sort((a, b) => a - b);
const hashTarget = join(import.meta.dir, "..", "src", "native", "rtti-class-hashes.json");
writeFileSync(hashTarget, JSON.stringify({ source: "FNV-1a 32 of every class name in an RTTI dump (red-dump-json layout); generated by tools/native-rtti-subset.ts",
  classes: classNames.length, hashes: hashes.map(hash => hash.toString(36)).join(",") }) + "\n");
console.log(`${classNames.length} class names → ${hashTarget} (${statSync(hashTarget).size} bytes)`);
if (args.includes("--hashes-only")) process.exit(0);
