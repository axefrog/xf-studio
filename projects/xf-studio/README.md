...
The V's skin, face details, eyes, brows, lashes, hair and piercings render in the preview from the player's own installed files: the loaded save's choices (or the creator's default V), vanilla, CCXL or a framework that replaces a vanilla style, resolved by the generic resolver through the launch route Build uses and exported from the winning archives. Loading another save replaces them completely. Any piercing style and colour the installation offers can be tried on the shown V without changing it; a tried style swaps in within about a second once its files have been read. These are browser material approximations, not verified game shader matches; hair physics is not simulated, and a rigid framework part does not follow the idle. See [save research](../../research/eye-artistry/save-import.md), [head CC rendering](../../knowledge/head-cc-rendering.md), [hair preview limits](../../research/eye-artistry/saved-v-hair-preview.md) and the [layered material evidence](../../research/materials/multilayered-shader-evidence.md).
