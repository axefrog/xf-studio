...
**Status:** phase 2 is built and tested offline: one command catalogue with a stdio MCP server, a command line and a JSON session runner on top, external window capture with crops and downscaling, and write methods for photo mode, the character creator's mirror screen and the world clock, all refused unless `allow_writes = true`. The plugin builds, the bridge core passes its offline self-test through every client, the redscript layer lints against the game's own script bundle and the Lua layer parses. **First in-game run, 26 September 2026:** the plugin, pipe, token, allowlist, game-thread queue, native reads and the redscript `Status` action worked; calls into redscript that reached TweakDB or `TDBID` natives crashed the game, because the plugin started scripts with no context. Every game and script call now goes the way CET makes them (a caller frame and a context, [design §3.5](../../research/runtime/runtime-bridge-design.md#35-calling-game-and-script-functions-from-native-code)); built and tested offline, not yet in game. The [test card](../../research/runtime/runtime-bridge-test-card.md) starts with a short script-call check, then the first-session checks and session 2 through the bridge. Design, source citations and the capability matrix: [research/runtime/runtime-bridge-design.md](../../research/runtime/runtime-bridge-design.md).
...
- **Unit checks** (`xfb_selftest --unit`) include the caller frame's parameter code for game calls (`core/ScriptFrame.cpp`, byte for byte as CET writes it) and the partial `photo.camera.set {reset: true}`.
...
The diagnostic zip differs from the default only in `enabled = true`; the writes zip also sets `allow_writes = true` and keeps all three write classes.
