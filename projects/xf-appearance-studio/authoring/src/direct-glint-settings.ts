...
export type DirectGlintFlakes = {model:"uv-cell-direct-1" | "uv-cell-direct-2"; density:number; fineShare:number;
...
export function isDirectGlint(value:unknown):value is DirectGlintFlakes{
  ...
  return Object.keys(f).sort().join()==="color,density,fineShare,model,seed,strength" &&
    (f.model==="uv-cell-direct-1" || f.model==="uv-cell-direct-2") && Number.isFinite(f.density) && f.density>=0 && f.density<=1 &&
