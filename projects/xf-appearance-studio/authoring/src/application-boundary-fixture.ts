...
import { applyLayerAction, layerCapability } from "./editor-actions";
...
const undo = () => { const recipe = documentState.undoRecipe(); if (recipe) documentState.recipe = recipe; };
...
const app = new StudioApplication({ document: documentState, recipe, gestures, controls, quality,
  layer: action => {
    const allowed = layerCapability(documentState.recipe, action);
    if (!allowed.available) throw Error(allowed.reason);
    const next = applyLayerAction(documentState.recipe,
      documentState.recipe.layers[documentState.active]?.id, action);
    documentState.checkpoint(); documentState.recipe = next.recipe;
    documentState.active = next.active;
  } });
...
function paint() {
  ...
  $("state").textContent = JSON.stringify({ presetCount: draft?.collection.presets.length ?? 0,
    ...
    removePoint: remove?.capability, actionCount: app.actionKinds().length,
