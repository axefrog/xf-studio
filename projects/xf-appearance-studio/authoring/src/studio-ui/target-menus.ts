...
const labels: Record<string, [string, IconName?]> = {
  ...
  "layer.remove": ["Remove layer", "trash"], "layer.rename": ["Rename…", "rename"], "layer.move": ["Move to position…", "arrowDown"],
...
const undoHint = (undo: string, id: string) => id.endsWith(".remove") || id === "preset.remove"
    ...
    { kind: "submenu", label: "Finish", icon: "finish", items: () => port.authoring.choicesFor(target, "layer.setFinish", "finish")
      ...
      .map(choice => {
        ...
        return { kind: "action", label: descriptor?.label ?? String(choice.value), capability: choice.capability,
          ...
          hint: descriptor?.exportAdapter === "none" ? "Preview study · not exported" : undefined,
