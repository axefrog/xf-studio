...
export function shell() {
  return section("shell", "02", "Shell and navigation",
    ...
    pattern({ id: "s-header", title: "Application header", status: "implemented", wide: true,
      ...
      drives: `${code("library.summary()")}, ${code("authoring.capability({kind:'recipe.undo'})")}, ${code("authoring.requestCapability({kind:'save'})")}, ${code("status.snapshot().verification")}, ${code("preferences")}.` }),
