...
New recipes use `xfs/recipe-2` for 0–32 ordered layers. Legacy `eye-artistry/recipe-1` four-layer recipes still load and normalize in memory; existing SQLite revisions remain byte-identical. New saves append the new format. Older studio builds cannot read recipe-2. Browser workspace keys, SQLite schema and collection IDs remain unchanged. Preset accordions with add/remove/reorder above the layers are still queued; the existing library dropdown is temporary.
...
[Layer/layout verification](evidence/editable-layers-2026-09-23.json): 37 tests, typecheck/build and actual browser add/copy/rename/drag/remove/empty/reload/Undo checks passed. Preset accordion work remains separate.
