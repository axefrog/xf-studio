...
- Use the closure slider or Blink to assess motion. The current pose is exploratory, synthesized from existing eyelid bones (44 in the head rig; 117 including accessory rigs). It closes the tested eye-shape-09 view with lashes, but is not extracted game animation and is not yet calibrated across all 21 eye choices.
...
- **Look library:** enter a name and Save look to store the whole four-layer composition in SQLite. Opening an existing look lets Save look append a revision; Save a copy creates a separate ID. The menu only selects an entry; Open look loads it. Loading is undoable. Concurrent stale saves are rejected instead of overwriting a newer revision. On page reload the browser draft is retained, but no library entry is implicitly selected for overwriting; explicitly open it to continue its revision history.
- Export recipe downloads editable JSON. Open recipe restores it and detaches from any library entry so the next Save look creates a new entry. Local browser storage still keeps an automatic draft; **SQLite saves are explicit for now**. Undo covers recipe edits for this session; no redo yet. Load V changes the preview head independently of the makeup recipe.
...
- **Load V from save** accepts a local `sav.dat`, entirely in the browser. It reads the appearance node and applies supported female facial morphs to the head, all four plates and corresponding accessory morphs. The card distinguishes applied face geometry, matching brow/lash reference resources, and unresolved skin, eye, hair and makeup resources. Export appearance data preserves decoded references. No save bytes are sent to the server. Imported V is session-only; reload and load the save again. The portable recipe currently stores makeup, not the imported V.
...
- `src/main.ts`: editing, draft persistence, recipe import/export and UI.
