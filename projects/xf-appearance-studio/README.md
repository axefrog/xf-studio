...
The first game package slice now exists: [Experiment 005](../../experiments/005-preset-collection/README.md) contains ONE selector with two complete overlapping-layer presets and Off, sharing one material template and one mesh/morph pair. A four-preset collection has authored lower mip chains whose decoded XBM texel-centre coverage is measured against the original base maps. A separate [local package command](../../research/authoring/local-package-build.md) accepts the Studio's editable collection export, validates it, builds in isolated intermediates and promotes only independently verified files to ignored project `dist` with a manifest. No package has been installed or render-verified. A numerically clear plate correction failed after resource packing, so the owned master remains unchanged. Next prove game selector registration, A/B/Off clearing, posed clearance and save identity while developing the remaining optical finishes. Editable collection accordions, SQLite snapshots and portable source export work. Preserve zero-offset and controlled-offset comparisons; do not regenerate a full combinatorial catalogue.
