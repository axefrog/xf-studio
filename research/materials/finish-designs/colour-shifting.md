...
- **Base colour through a texture, not a constant.** The base colour goes into a uniform 16 × 16 sRGB `GradientMap` (so its encoding is certain), with `DiffuseColor` white; `MaskTexture` carries linear coverage and needs no square-root encoding. Roughness 0.32 and metalness 0.08 come from `Scale` 0 plus `Bias`, so no surface maps are written. The metalness was 0.25 until 27 September; at 0.08 no covered pixel reaches the 0.1 at which skin leaves subsurface scattering, so the finish no longer shares Metallic's hard look ([decal reference §10](../shader-decal.md#10-recommended-changes-ranked), item 2). The tint is added to the base colour and needs no metalness.
