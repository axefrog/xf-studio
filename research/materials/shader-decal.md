...
| XF Eye Artistry (Colour-shifting) | `mesh_decal_gradientmap_recolor_blendable`, `@fresnel_<preset>` | Uniform base, Fresnel tint, roughness 0.32 and metalness 0.08 by bias (§8) |
...
|---|---|---|---|---|
| **Matte** | `mesh_decal`, `@preset` | roughness 1.0 (was 0.88, §10 item 5), metal 0, surface alpha 1 | §4.3 squared coverage (√ alpha), §4.6 surface write, §6 dual lobe | Replaces skin's pore-level roughness with a constant; vanilla matte writes the same 1.0, but at partial weight (§3) |
...
| **Colour-shifting** | `…_gradientmap_recolor_blendable`, `@fresnel_<preset>` | uniform base, `FresnelColor` linear-normalised, roughness 0.32, metal 0.08 (was 0.25, §10 item 2), fade pushed to 1000 m | §5.2 Fresnel before √, linear coverage, no UV transform; §6 SSS switch | At 0.08 no covered pixel reaches the 0.1 SSS switch, even at full coverage; the tint itself is added to albedo and needs no metalness |
| **Glitter** (diagnostic board) | `mesh_decal` mode 1 + `NormalAlphaTex`; accent on `mesh_decal_emissive_subsurface` | flakes tilted to 50°, `EmissiveEV` 1 on the accent (was 0, §10 item 1) | §4.5, §7; §5.4 for the accent | The accent writes the flake colour itself; its opacity is still an engine modifier times the mask (§5.4), so it may draw nothing |
...
2. **Matte cannot be glossier than skin under direct light.** At the session-1 value 0.88 its lobes were 0.85 and 1.41, dimmer than the skin's (at today's 1.0 they are 0.97 and 1.6). If Matte still reads glossy in *Gloss A*, the cause is not its written roughness: *Gloss B* and *C* then separate the remaining suspects (close-up breakup showing skin through, fixed by the lift and confirmed in session 2 [runtime]; photo-mode lights' per-light roughness shift; probe and screen-space reflections, not traced).
...
**Before the Gloss, Shimmer and Metal verdict session.** Nothing here invalidates the staged session-2 build: its *Gloss A–D*, *Shimmer · strong* and *Metal ramp* presets are the right controls, and §13 says how to read them. Two findings change what the verdicts can mean: Metallic's hardness is the SSS switch, which no roughness value removes (§9.1 item 4), and Shimmer's facets need to clear the 11.5° gate, which *Shimmer · strong* mostly does and the default does not (§9.2). The staged session-2 build predates ranks 2 and 5 (its Matte stripes write 0.88; it has no Colour-shifting preset); a rebuild from today's code would carry Matte at 1.0, and a tilt-floor Shimmer stripe (rank 3) would be cheap to add then.

1. **Done: glitter accent (before the glitter board is staged).** Set the accent's `EmissiveEV` above 0: this template multiplies `EmissiveColor` by it (§5.4), so 0 is black. Add to the test card that the accent's opacity is `MaterialModifiersConsts[2].x` × mask, so "no accent at any EV" means the engine does not drive that modifier on a CCXL component, not that the mask is wrong.
2. **Done: Colour-shifting metalness 0.25 → ≤ 0.08.** The tint is added to albedo and needs no metalness; below 0.1 the covered area keeps the skin's SSS instead of turning hard like Metallic, and no seam appears at 40 % coverage. Safe before the verdict session: Colour-shifting is not among the finishes it judges.
...
5. **Done: Matte 0.88 → 1.0** whatever the *Gloss D* verdict: nothing in the light rewards 0.88 over 1.0, and vanilla matte lipstick writes 1.0.
...
**Coordinator decisions (26 September 2026, provisional, for the maintainer's review):** items 1, 2 and 5 are applied as one finish-defaults change. **Item 1:** the glitter board's accent writes `EmissiveEV` 1. Because this program multiplies `EmissiveColor` by `EmissiveEV` directly (§5.4), 1 writes the flake colour itself, the brightest value that keeps every channel inside the colour's own 0–1 range: visible wherever the engine's modifier lets the accent draw, with no over-range value for bloom to pick up. Values between 0 and 1 only dim it. The diagnostic knob and the verifier both refuse an accent at 0 or below. **Item 2:** Colour-shifting writes metalness 0.08, under the 0.1 switch at any coverage. **Item 5:** Matte writes roughness 1.0. The preview follows both values. Item 3 (the Shimmer rebake) waits for the *Shimmer · strong* verdict; items 4, 6 and 7 stay proposals.
...
- **Glitter board, step E (when staged)**: the accent now writes `EmissiveEV` 1, so "no points at all, even in the dark" answers open question 1 (the engine leaves `MaterialModifiersConsts[2].x` at 0 on a CCXL component).
