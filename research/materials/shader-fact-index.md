...
**When a fact changes:** update its source page, then every file in its row, then the tests those files name. Rows marked **changed in this study** are the ones the [skin](shader-skin.md), [hair](shader-hair.md) and [eye](shader-eye.md) references confirmed or corrected on 26 September 2026.
...
| Fact | Grade | Documented in | Preview | Export / verify |
|---|---|---|---|---|
| Multilayered: per-pixel front-to-back coverage shares, levels as clamped scale/bias, colour mask from the roughness map; no refraction or Eye light on layered eyes | [observed]; CPU mappings [hypothesis] | [multilayered evidence](multilayered-shader-evidence.md) | `layered-material.ts`, `layered-setup.ts` | — |

Related: [skin reference](shader-skin.md) · [hair reference](shader-hair.md) · [eye reference](shader-eye.md) · [materials and shaders](../../knowledge/materials-and-shaders.md) · [backlog](../backlog/materials-shader-re.md).
