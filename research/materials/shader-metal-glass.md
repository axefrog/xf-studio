...
- **Ambient and reflections:** the full-screen composite adds `albedo × E` (the diffuse-irradiance texel) and a reflection from the reflection target through a split-sum lookup texture, with specular occlusion `saturate(3·GBuffer2.z)`, which is 1 for Standard ([hair reference §6.5](shader-hair.md#65-environment-path)) [observed]. What fills the reflection target (probes, screen-space or ray-traced reflections) is not traced [hypothesis].
...
| | **Creator preset: no environment** (`lighting-preset-stage.ts` sets `scene.environment = null`) | Every metal (these parts and the layered piercings) shows only the spot lights' highlights and is otherwise near black; in game, Standard pixels also take the ambient composite's probe diffuse and reflection [observed program], though what the creator scene's probes hold is unknown [hypothesis] |
...
2. **An environment for the creator preset (S–M).** Give the creator preset a dim prefiltered environment (the studio room scaled, or a capture of the creator backdrop) with one intensity calibrated against a creator capture of a metal part. Without it, rank 1's metals and every layered metal read black in that preset. This is a lighting-stage change, not an adapter, and affects every Standard part.
