...
- **Radiance.** The model is scaled by `2π × E`, where `E` is the pixel's diffuse irradiance, the same texel that lights Standard pixels as `albedo × E`. Hair does **not** read the reflection target or the split-sum lookup.
