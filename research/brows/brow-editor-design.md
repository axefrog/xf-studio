...
| Chart | Flattened chart of 458 head triangles per side, needing domain, collapse and seam checks | The brow's own UV, already flat, shared by both brows, u not flipped [resource]. Not isometric (phase 0): 53–93 mm per unit along the brow and 28–44 across (female, 5–95 %), so a 2048 × 1024 texel is 0.026–0.045 × 0.027–0.043 mm [offline] |
...
- The chart is the brow's UV area in glTF UV (1 − stored v; image rows top-down show the brow upright), u running medial to lateral on both brows, and the working plane is millimetres. Tracing uses the **per-triangle Jacobian** (positions against UVs) so that a 6 mm hair is 6 mm on the face wherever the UV stretches, and root density is per face-mm² (the earlier project's "fixed neutral area" idea).
- **Phase 0 result:** the strip is far from isometric, so the per-triangle metric is required. Against one constant scale (75 × 36 mm per unit) the median female triangle's worst length error is 22 %, only 39 % of the area is within 20 %, and the upper edge runs 88 mm per unit u against 55 at the lower edge [offline]. The chart builds the metric at load time from the neutral brow mesh the host already loads (664 triangles, negligible cost), so no table of game geometry is stored or committed; phase 1's tests use synthetic charts.
- **Both genders share the textures but not the UV footprint or the metric.** The male strip spans u 0.029–0.924, v 0.111–0.955, against the female's u 0.022–0.985, v 0.162–0.979 [resource]. Their intersection is 42 % of the texture (80 % of the female footprint, 96 % of the male), bbox u 0.029–0.924, v 0.162–0.955, and every vanilla creator style already lies inside it [offline]. The chart panel draws both footprints and the vanilla content box (the union of the creator styles: u 0.03–0.97, v 0.16–0.71) as guides. The domain check refuses outlines outside the intersection, eroded by a few texels so mips do not bleed past it, with a plain message ("Keep the brow inside the area both body types show"). At the same texel the male strip is 0.65–1.41 × the female's length along the brow and 0.99–1.59 × across (5–95 %; medians 0.96 and 1.21) [offline], so one texture cannot be millimetre-exact on both bodies. Default (phase 0): millimetres refer to the **female strip** as the reference chart for every style, whatever the V's body; the preview draws the V's own body gender, so a masculine V shows the true, about 20 % taller result. Per-gender texture sets would make both exact at twice the size; they are not planned.
...
| `_d` RGB | Greyscale tone (§3.6) | sRGB-tagged (`isGamma` 1), so the encoder targets *linear* values. Vanilla sits at linear 0.30 on average under coverage (phase 0) | [resource] |
| `_ds` alpha | Powder underlay | Vanilla ships 64 × 32 BC7; we would ship 256 × 128 (a soft term needs little resolution). The program reads only `.a`; `_ds` RGB is never used (phase 0, [source]). **Caution:** the secondary term is tinted by the material's constant `SecondaryDiffuseColor`, not the gradient [source, via the [brow material study](../eye-artistry/brow-lash-fidelity.md)], so heavy powder would stay brown for a blonde brow. The default is therefore `_ds` = 0, with powder folded into `_d` alpha through the inversion so that it follows the colour row. `_ds` stays available for an explicit "tinted underlay" option. | [source] |
| `_n` RG | Hair relief | Each hair as a cylinder lying on the skin: the normal tilts across the hair's width perpendicular to its direction, weighted by coverage, and is **exactly flat** (128, 128) elsewhere: vanilla's `UseNormalAlphaTex` 1 with the default white `NormalAlphaTex` writes the normal at `NormalAlpha` × the tilt gate whatever the coverage, so any tilt outside the hair would land on bare skin. Strength goes into the style's `NormalAlpha` (vanilla 0.4, Arkhe 0.8). Frame (phase 0): red tilts toward +u (lateral), green toward the bottom of the upright glTF-UV image (down the face); the mesh's tangent sign flips with the mirrored UV, so one map lights both brows. Vanilla and both mods' maps follow the same convention. The in-game arrow test (test ask 3) still confirms it. | Encoding [resource]; frame [source] + [resource] |
...
**Mips.** Because the program squares *after* filtering, a plain box mip of `p` darkens nothing but thins the brow at a distance: the mean of `p`, squared, is less than the mean of `p²`. The exporter therefore builds its own chain from the linear coverage: level `n` holds `√(mean C)` over its footprint, which preserves each level's mean on-screen coverage. Arkhe's installed chain and vanilla's are plain box reductions of alpha [resource] ([mip gate](../eye-artistry/brow-lash-mip-gate.md), phase 0), and vanilla brows measurably thin with distance: mean on-screen coverage falls to 0.69–0.94 of level 0 at mip 1 and 0.44–0.84 at mip 4 [offline]. This is an improvement to test, not a copy (test ask 2).

**Sizes and format.** At 2048 × 1024, BC7 with mips is about 2.7 MiB per map, about 5.4 MiB per style for `_d` and `_n`; at 1024 × 512 it is about 0.7 MiB per map. Arkhe ships uncompressed RGBA chains (about 10.7 MiB per map) [resource]. The Studio should use BC7 for `_d` and BC5 (`TCM_Normalmap`, two channels) for `_n`, as vanilla does (phase 0); each is 2,796,240 bytes at 2048 × 1024 with 12 mips. Hair edges at 0.035 mm per texel survive BC7; a side-by-side check of the encoded result is part of phase 1.

### 3.6 Colour

- **Creator's row (default, decision 2).** The program's colour is gradient × `GradientMapIntensity` × primary RGB [source]. The style's own `.mi` (ArchiveXL's `BrowsBaseMaterial`) keeps the vanilla intensity of 2. **Tone level (changed in phase 0):** vanilla `_d` RGB averages linear 0.30 under coverage (0.23–0.42 across the 13 creator styles), so vanilla brows show about 0.6 × the gradient colour; a tone centred on 0.5 would reproduce the gradient itself but look 1.7 × brighter than a vanilla brow in the same creator colour. Mods disagree (Even More Brows ≈ 1.0, so 2 × the gradient; Arkhe II 0.55 at intensity 0.5, so 0.28 ×). The default tone base is therefore **linear 0.30**, so an XF brow sits beside vanilla brows in every colour; the base stays a style parameter. The gradient is sampled with clamp at its last texel, and at intensity 2 a channel above 0.5 clips: that happens in 7 of the 35 colours (`blue_sky`, `blue_steel`, `citrus_yellow`, `green_orange`, `phoenix_fire`, `purple_blonde`, `red_apple`), which shift hue slightly; tone variation still survives [offline]. Per-hair tone variation (a few hairs lighter or darker), root-to-tip lightening and a **lightness ombre along the brow** (for example a softer, lighter front and a defined tail) are all greyscale, so they survive every colour, including installed hair-colour packs that patch the brow scope. Arkhe's style-18 `_d` has identical greyscale RGB channels [resource] ([texture audit](../eye-artistry/brow-texture-audit.md)), which confirms that mods ship tone this way.
...
- **Prerequisite:** the adapter still draws no brow normal and a fixed roughness of 0.8 ([eyebrows §5](../../knowledge/brows.md#5-what-the-studio-draws-today)). Moving brows onto the face-decal family's double-diffuse member (`src/face-decal-material.ts`, which already writes both) is required before the preview can show relief or match brow gloss. The GLB export negates the tangent sign along with v, so a Three.js material built on the exported tangents must negate the map's green (or w) to light relief as the game does (phase 0).
...
- **Labels and entries (phase 0).** The row label is the switcher choice's `localizedName`, plain text in vanilla (`01`…) and both mods (`EMB 01`), so "XF <look name>" is written there. ArchiveXL merges switcher choices by that text and a later duplicate **replaces** the earlier one, so the exporter keeps labels unique within the collection and refuses one that equals a vanilla or installed label. Each style's appearance option copies the vanilla fields (`uiSlot` `eyebrows_color`, `link` `eyebrows color`, `linkController` 1, `randomizeCategory` `Eyebrows`, `LocKey#43266`, `useThumbnails` 1, a `Soft` app) with the 35 vanilla colour definitions, and is listed in the three head groups; option and definition names carry the `xfs_` prefix.
...
|---|---|---|---|
| 0. Facts | **Done 27 September 2026** ([experiment 025](../../experiments/025-brow-facts/README.md)): the per-triangle metric of both strips (not isometric), UV orientation, footprints and their intersection, vanilla and mod texture formats, mips and tone levels, the secondary term, gradient sampler and clip, the tangent frame and green convention, the creator entries. Changes to this design are marked "(phase 0)" below. | 1 | Findings recorded in [eyebrows](../../knowledge/brows.md) |
...
Offline: done in phase 0 ([experiment 025](../../experiments/025-brow-facts/README.md)). Test ask 3 now checks a predicted convention (green toward the bottom of the upright image), and ask 4 should also compare hair length on both bodies (predicted: about 20 % taller on the masculine strip).
