...
**Verified so far.** Unit tests cover the version, tag, changelog, staging and notes rules. A PowerShell rehearsal of the build job ran in a fresh shallow clone with an empty Hutch home and no assets: installs, devkit preparation (130 s, mostly downloads), both typechecks, `bun test` (455 pass, 5 private-asset skips), `build:canary` with the allowlist gate (31 s) and staging all passed, producing a 35.4 MB setup ZIP for `0.1.0-alpha.1`. The workflow itself **has not yet run on GitHub**, so the first manual run is the real CI proof. Two local findings:
