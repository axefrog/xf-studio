...
| Download | One self-contained setup program, `XFStudio-<version>-win-x64-setup.exe`, with nothing to extract. Electrobun's own setup only works beside its hidden `.installer` payload folder, so CI wraps Electrobun's unmodified setup and payload in a pinned Inno Setup program that runs it; the Electrobun setup ZIP is a build input, not a release asset. See [single-file setup](../../projects/xf-studio/authoring/desktop/README.md#single-file-setup). |
...
| Manual run (`workflow_dispatch`) | Same | 14-day workflow artifact: the single setup program, `build-info.json`, `THIRD_PARTY_NOTICES.md`, `SHA256SUMS.txt` |
| Pushed `v*` tag | Same, plus: tag equals the version and the changelog section is complete; then a separate job checks the tag is on `main` and has no existing release, re-verifies the checksums after transfer, composes the notes, attests the setup program, `build-info.json` and the notices, and creates the draft | Draft GitHub release (pre-release for `-alpha`/`-beta`/`-rc`) |
...
- **Installer wrapper.** Inno Setup 6.7.3 (`desktop/inno-setup.ts`) is downloaded from its official GitHub release, accepted only with the pinned SHA-256 and unpacked in its portable mode into a job-local folder; `desktop/installer/xf-studio-setup.iss` is the whole script. The wrapper is per-user (`PrivilegesRequired=lowest`), installs nothing itself and registers no uninstaller, so Electrobun's setup still owns the install folder, shortcut, uninstaller and the retained payload a future updater needs.
- **Release assets:** `XFStudio-<version>-win-x64-setup.exe` (the single setup program), `build-info.json` (commit, version, Electrobun build hash, toolchain, the Inno Setup release, `signed: false`, `updater: "disabled"`, `includesGameAssets: false`), `THIRD_PARTY_NOTICES.md` and `SHA256SUMS.txt`; the first three carry attestations. The setup ZIP, update archive and `update.json` are **not** published, so no release can serve as an accidental update feed.
...
**Technical caveat for any signing route.** Electrobun 2.0.1 does not sign Windows output. The download is an Inno Setup program carrying Electrobun's `XF Studio-Setup-canary.exe` plus a `.tar.zst` payload holding the unsigned `launcher.exe`, `bun.exe` and native DLLs. Signing only the outer setup program removes the download warning but leaves Electrobun's setup and the installed binaries unsigned, which Smart App Control can still block. Signing the inner binaries needs the `postWrap` hook (which receives `ELECTROBUN_WRAPPER_BUNDLE_PATH`) to sign them before compression, then Electrobun's setup executable, then the Inno Setup program (Inno Setup can call a signer itself through `SignTool`). `verify-canary.ts` compares the wrapped files with the ZIP byte for byte, so it must run on the signed files in the same order. That fits a synchronous signer such as Azure or a cloud-HSM `signtool` in the same job. SignPath signs asynchronously outside the job, so it would need the build split around that hook, or SignPath's support for signing files nested in the payload. Prove either with a disposable identity before relying on it.
