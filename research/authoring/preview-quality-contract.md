...
The active point follows its surviving layer across rename, move and Undo, and is clamped when the layer has fewer points. Focused service, browser-device and Three stack tests verify this boundary. In the primary checkout, all 406 Studio tests, typecheck and browser build passed with private fixtures available. An isolated live `?verify=1` session selected point index 2, renamed its layer, then undid the rename: the selected point stayed index 2 and preview readiness stayed *Ready* throughout. The worktree's earlier five fixture-absence failures were not implementation failures.
