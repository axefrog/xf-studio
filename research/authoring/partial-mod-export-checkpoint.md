...
**Status (25 September 2026): current policy.** The finish filter below is still what Check and Build use. Exportable finishes come from the route policy in `projects/xf-studio/authoring/src/engines/layered-makeup/finish-export.ts`: Matte, Satin (internal `regular`) and Metallic, plus experimental game-matched Glossy, Shimmer and Colour-shifting (one colour-shift pigment per preset). Glitter layers and earlier-model layers are omitted with warnings that name the reason, and Check/Build list experimental layers. Check/Build — in the browser Studio, the local CLI and the installed desktop trial — produce independently verified **private candidates**. A diagnostic MO2 profile has the four-preset (Matte/Metallic/Off) candidate staged; a five-preset candidate adding a Satin control is built but not staged. **No in-game test has happened yet**; the maintainer runs it from the [prepared session card](first-makeup-runtime-preflight-2026-09-25.md). Since platform step 8 the filter is eye makeup's exporter's eligibility: Check and Build report omissions per feature and per mod (a part of a feature with no exporter is reported once, a feature with nothing left is reported and left out while the others build), and nothing usable left is still refused ([step 8 status](feature-module-platform.md#step-8-status)). The maintained end-to-end description is [the Studio-to-mod pipeline guide](studio-to-mod-pipeline.md). The evidence below is the original 24 September verification of the filter.
