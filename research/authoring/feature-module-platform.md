...
- **IDs.** `featureId()` and `familyId()` keep the literal type and accept lower camel or kebab case (`eye-makeup`, `savedV`). A qualified ID is `<owner>/<kind>` (`eye-makeup/layer.setColor`, `history/recipe.undo`, `history/history.undo` since step 5). Commands keep their flat, grandfathered kinds. The `{ kind: "feature" }` envelope waits for module #2, because no current action needs it.
- **Undo policy name.** The policy stays `recipe` rather than `part`: step 4 kept it, because the golden registry snapshot and the presentation port carry the value; it is renamed with the presentation facade (step 5).
...
- **Generic feature handlers and `app.transaction(label, features, fn)`** (`StudioApplication`). A registered feature without a bespoke handler runs its spec's `capability` and `apply` over its live document and records one `part` step by its Undo policy. A look transaction opens one `look` step over the named parts (the platform's `HistoryTransaction` with the form-control policy), runs `fn` with per-action steps suppressed, keeps nothing when nothing changed and reverts without Redo when `fn` throws or returns a failure. Undo, any other owner's action, gestures, form controls and a nested transaction are refused inside it.
- **View contributions** (`studio-ui/views/`). The shell's view and eye makeup's view each list their panels (ID, title, icon, purpose, catalogue order, shell slot, where a closed panel opens, whether it repaints lazily) and activity sources; `views/panels.ts` binds each panel to its factory, keyed exactly by that view's IDs. `PANEL_IDS`, `PANEL_META`, the factory layouts, `CLOSED_PANEL_HOMES`, the lazily repainted panels and `sourceLabel` derive from them; the shell maps slots to tab groups per size class and drops slots nobody fills. The header's authoring category reads the registered features.
...
- **`port.editor` is removed** rather than kept as an alias: its one consumer is eye makeup's view, which now reads its facade, and two names for one view would be migration debt. `port.authoring` keeps its eye-makeup-specific methods for now (other callers and tests); they move behind the facade with the file moves.
...
- **The view composition list sits in `studio-ui/views/`** until the file moves, recorded as a boundary exception with its removal criterion in the [boundary assessment](ui-architecture-boundary.md#open-work): studio-ui may not import `compose/`, and eye makeup's panel modules have not moved.
...
- **A locked look is kept, never edited or written from the editor.** Selecting it shows an empty editor; every feature action, gesture, form control, look transaction and recipe or mask export is refused with `unavailable` and the reason; collection edits (rename, copy, move, remove, restore) work, and a copy is locked too. Every writer puts it back exactly: the workspace (parts and memory as read), collection files (always `xfs/collection-2`, since `legacyPreset` never writes a locked look as collection-1), the budget (a removed preset or recovery draft holding a locked look is never dropped to fit, and recovery drafts keep such removed entries) and the desktop store (which now reads with `keep`). The library never takes it: Save is refused up front and the store refuses it with the same plain message, so older rows are never rewritten; Export collection exports the draft unsaved through the CORE-38 path. The mod export omits it with "It was made with a newer version of XF Studio." The presentation shows the reason in Layers with "Get the latest version" (a new `project-releases` link), marks the look in the Presets list and names it in the viewport hint strip. Removing a locked look and later pushing it out of the 20-deep removal stack, or out of the four-deep recovery queue by opening other collections, still drops it as it would any look: those are the person's own actions.
...
**What remains of step 5:** the `git mv` of eye makeup's files into `features/eye-makeup` (its view under `view/`) and `engines/layered-makeup`, with the view composition list moving to `compose/` (import rewrites only), deferred while other agents work in the rendering files (`makeup-stack.ts`, `plate-blend.ts`, `scene.ts`, render records, `piercing-preview.ts`) and hosts (`resolver-host`, `grading-lut-host`, `eye-plate-service`, the verifier). Also open: eye makeup's commands and input bindings as view contributions (the palette's eye-makeup entries and `KEY_BINDINGS` are still shell code), tours and help topics as contributions (§6a), moving `port.authoring`'s eye-makeup methods behind the facade, and a presentation action that uses `app.transaction` (for example "Reset look" once module #2 exists).
