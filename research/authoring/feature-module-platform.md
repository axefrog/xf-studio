...
**Commands.** Platform families (collection/preset, history, camera/preview, motion, quality, saved-V, files, host detection) keep flat `kind` IDs, registered as *system families* in the same table shape as features. **Not yet registered** (CORE-36, a recorded gap): library requests (`REQUEST_DESCRIPTORS`, run by `CollectionService.execute`), file workflows (`FILE_DESCRIPTORS`, run by `StudioFileOperations`) and gesture proposals (`GESTURE_DESCRIPTORS`). They are catalogued through `actionRegistry()` beside the registry's own kinds, but routed by hand: they are asynchronous (requests, files) or run inside a gesture session, and the registry's `ActionHandler` is synchronous and one-shot. They join as families when the look history (step 4) gives the platform transactions and async work a home; eye makeup's gesture frames already apply through its registered `gestures` provider. Feature commands use an envelope:
