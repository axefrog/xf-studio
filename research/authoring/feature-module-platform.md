...
- **Look transactions.** One `look` entry containing several parts (`LookHistory.checkpoint(read, features, { scope: "look" })`). Examples: "Apply character from save", "Paste look", "Reset look". The app-level `app.transaction(label, features, fn)` waits for a second editable feature ([step 4 status](#step-4-status)).
