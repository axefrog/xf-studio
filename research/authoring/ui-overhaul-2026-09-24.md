...
|---|---|---|---|---|
| B-1 | Opening or importing a second collection permanently discards the earlier unsaved draft — the recovery slot held one draft | `collection-session.ts`, `collection-workspace.ts` | **Reproduced before fix**: after opening A then B, undo-open alternated A/B; "Draft X" was unrecoverable | **Fixed in follow-up, 25 September**: four browser drafts form a persisted recovery queue. Repeated recovery reaches earlier edits, Undo history and revision metadata; the draft left behind remains reachable. Opening at capacity warns before evicting the oldest draft. SQLite revisions are unchanged. |
| B-2 | Layer rename/move/duplicate/reset, every Undo and slider Escape rebuild every layer texture and reset the selected point to 1 | `authoring-layer-actions.ts`, `trusted-authoring-core.ts`, `authoring-document.ts` | **Reproduced before fix** (browser, 2K): rename reset point 3→0 and re-queued all 4 textures (`updating`, pending 4) | **Fixed in core, 25 September**: rename/Undo keep completed textures; move and structural Undo follow stable layer IDs; changed/new or unfinished layers render individually. A surviving selected point stays selected, clamped only when its contour shrinks. Full collection/preset restore and quality changes still replace the stack. |
| B-3 | A refused file action clears the in-flight action's `busy` flag, re-enabling a second bake mid-bake | `studio-file-operations.ts` | **Reproduced before fix** (service): `busy` became `undefined` after a refusal; `mask.export` capability returned available mid-bake | **Fixed in follow-up, 25 September**: refusals return their reason without changing `busy`/`last` or publishing a state transition. The acquired action alone can clear its transaction; an overlapping bake stays refused until its result, error or cancellation completes. Focused concurrency test covers refusal, notification, subsequent success, error and cancellation. |
