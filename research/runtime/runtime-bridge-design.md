...
| Restore on kill | After the kill switch, once the queue is closed (`Bridge::RestoreReady`), the next Running tick runs `RestoreAfterKill` once: unfreeze the world, show the photo-mode menu; the save lock stays. The ordering (no queued write after the undo) is [offline] in the unit test; the trigger itself is plugin-only code with no offline test yet | [unverified] |
