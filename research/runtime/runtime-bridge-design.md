...
**Status: phase 2 built and tested offline; first in-game run on 26 September 2026.** The transport, token, allowlist, the game-thread queue and the plain native reads worked in the game, and so did the redscript `Status` action; calls into our redscript that reached TweakDB or `TDBID` natives crashed the game. The cause (a script call with no context) and the fix are in §3.5; the fix is built and tested offline and awaits the [script-call check](runtime-bridge-test-card.md#script-call-check-first). The plugin DLL (0.2.0) builds, and the bridge core passes an offline self-test through every client, including the cases added for the security review ([code-health ledger](../authoring/code-health.md), RB-01..11). Phase 2 adds one command catalogue with an MCP server, a CLI and a session runner (§3.4), external capture, and write methods behind `allow_writes` (§3.2). The redscript and Lua layers pass offline checks. Nothing here has been observed in the running game yet; the [first-session test card](runtime-bridge-test-card.md) asks for that, then runs session 2 through the bridge. Project: [`projects/xf-runtime-bridge`](../../projects/xf-runtime-bridge/README.md). Consolidated answers live in [knowledge/runtime-access.md](../../knowledge/runtime-access.md).
