...
Error codes: `bad_request`, `bad_params`, `bad_version`, `unauthorized`, `killed`, `rate_limited`, `unknown_method`, `writes_disabled`, `game_not_running`, `timeout`, `timeout_after_start`, `busy`, `too_large`, `game_not_ready`, `rtti_missing`, `rtti_signature`, `call_failed`, `script_calls_unavailable`, `script_layer_missing`, `not_in_photo_mode`, `not_in_gameplay`, `not_in_character_menu`, `unsupported`, `unavailable`, `failed`. Implemented in `native/src/core/Dispatcher.cpp`, `PipeServer.cpp`, `core/Params.cpp` and the redscript actions layer; the tools' plain wording for each is in `tools/api/errors.ts`.
...
| `photo.hud.hide` | write-photo | game | Our `@addMethod` on the controller around its protected `OnFadeVisibility(Float)`; with `cursor` (default), a flag that a wrap of `CursorGameController.ProcessCursorContext` reads to play `Hide` instead of any context, only while photo mode is active, replayed on every cursor controller seen when the flag changes (the technique learned from Appearance Menu Mod; knowledge/photo-mode.md §6). The flag is cleared when photo mode opens or closes, whenever `Status` sees another phase, after the pipe drops a client for idleness (`XFBridgeActions.ReleaseCursor`, from the next Running tick) and by the kill switch | fade [runtime]; cursor [source], in game [unverified] |
| `face.rig.read` | read | game | Finds V's photo-mode stand-in (as `photo.subject` does) or its head item (`TransactionSystem.GetItemInSlot` in the placement slot of `Items.PlayerWaPhotomodeHead`/`PlayerMaPhotomodeHead`, as photo mode places it), then each named component through our `@addMethod(Entity)` around the protected `FindComponentByName` (redscript `XFFace`). The plugin reads the component's animation setup through the game's reflection, property by property, only when the property exists with the expected kind of type: for an `entAnimatedComponent` `facialSetup` (raRef), `graph` and `rig` (rRef) and `animations`; for an `entAnimationSetupExtensionComponent` `animations`; each `animAnimSetup` as its `gameplay` and `cinematics` entries (`animSet` raRef, `priority`), at most 64 each. References are reported as their path hashes (first 8 bytes of both reference types); a missing or differently typed property is listed under `unreadable`, never guessed. No game function is called on a component | [source] 2.31 `photoModePlayerEntity.script:378-439`, `entity.script:32`, `orphans.script:18131`; field names and types: RED4ext.SDK generated types, `red-dump-json`, Codeware `AnimatedComponent.reds` (research/animation/expressions-evidence.md); in game [unverified] |
| `photo.expression.index` | write-photo | game | `new AnimFeature_PhotomodeFacial()` with `facialPoseIndex`, `AnimationControllerComponent.ApplyFeature(target, n"PhotomodeFacial", feature)` and `PushEvent(target, n"updateFacialPose")` on the stand-in (default) or its head item: the photo-mode face graph's own input and switch event. Refuses an index the photo-mode expression list (key 28) doesn't offer unless `unlisted`; reads the menu's expression first, and the undo is `photo.expression.set` with it. For the expression design's R2 | [source] 2.31 `orphans.script:48825-48828`, `animationControllerComponent.script:30-70`; input and event names [resource] the photo-mode face graph (knowledge/facial-expressions.md); effect [unverified] |
...
RED4ext removes a state callback that returns `true` [source] `StateSystem.cpp:128-160`, although the SDK comment says the Running result does not matter (`Api/v1/GameState.hpp`); the plugin therefore returns `false`. The Running `OnUpdate` runs on the game's main thread [runtime: the crash report's stack starts in RED4ext's entry frames], in the same state tick where CET runs its Lua `onUpdate` [source] CET `GameHooks.cpp:93-104`, `D3D12.cpp:65`. Script calls from it work at the main menu and in gameplay when they are made with a context (§3.5); loading screens are still [unverified].
...
- every engine address this path needs (the internal execute, and `CGlobalFunction_ctor`, `CClass_CreateInstance` and `Handle_ctor`, which making the caller and the context reach) resolved once at load through RED4ext's `RED4ext_ResolveAddress` (RB-32). The SDK would otherwise resolve them at first use and, for a hash the address library lacks, show a modal error and end the game; with one missing the plugin logs `script.addresses_missing` and refuses every call with `script_calls_unavailable`;
...
- **The creator from gameplay** (batch 3). `cc.open` opens the appearance screen without the player: the redscript layer checks the moment, takes the save lock, waits for the game to report saving locked, then raises its own event on the idle menu scenario through the game's menu-event blackboard, and the event switches to the mirror's scenario with an edit tag (mirror or ripperdoc). No vanilla function is wrapped; the event is inert unless the bridge asked for it in the last three seconds ([knowledge/photo-mode.md §3.1](../../knowledge/photo-mode.md#31-how-character-customization-anywhere-opens-it)). It shares `allow_creator_leave` with Confirm and Back. `cc.page` points the creator's camera, `cc.apply` takes values by label and options by slot, and `world.time.set` also works with the screen open (as the game's own time-skip menu sets the clock under a full-screen menu).
- **Settings for the record** (batch 3). `game.options.read` reads the graphics and display user settings in script and, through the CET layer, the engine's character render options, which no script can read: the plugin names the options (`XFBridge_OptionsWanted`), CET answers with `GameOptions.Get` (`XFBridge_OptionsReport`), and `core/OptionsExchange.cpp` accepts only an exact answer to the pending request. Read-only.
- **Expression research** (batch 2). The [expression editor design](../animation/expression-editor-design.md#7-the-runtime-bridge-and-in-game-preview) asks the running game two questions before it builds anything: which facial setup, graph and sets the photo-mode face uses (R1: `face.rig.read`), and whether the photo-mode `faceId` is the face database's index and which entity takes the face input (R2: `photo.expression.index`). Both are grounded in the 2.31 scripts; the design's other two commands (`face.expression.stage`, `face.controls.apply`) are not built.
...
| Visible indicator | CET draws "XF bridge: listening (read-only / writes ON)" whenever the bridge listens | [runtime] (the label turned red on the kill, [script-call check S7](runtime-bridge-test-card.md#script-call-check-first)) |
...
| Restore on kill | After the kill switch, once the queue is closed (`Bridge::RestoreReady`), the next Running tick runs `RestoreAfterKill` once through `writes::RestoreOnce` (only after a write; a failure is logged and not retried): unfreeze the world, show the photo-mode menu; the save lock stays. It is a redscript call like the others, so it uses §3.5's call path | Trigger and ordering [offline]: unit checks with a fake script caller, and the self-test host (restore logged exactly once after a write, never without one); in game the restore ran once with the world unfrozen and the save lock kept [runtime] ([script-call check S7](runtime-bridge-test-card.md#script-call-check-first)); showing the menu again [unverified] |
| One allowlisted input | `photo.open` is the only thing that sends input: the player's photo-mode key, to the game's own window (owner checked, and the owner's image must be `Cyberpunk2077.exe`), only when the bridge's write gate allows photo writes, V is in the world and the game says (explicitly) that it allows photo mode, checked again after the window comes forward and just before the key; `sendinput` refuses unless the game window is really in front, so the key can't land in another program. An unbound, malformed or unreadable binding and layout-dependent punctuation keys are refused, never guessed; navigation keys go as extended keys. The test-only window override counts for keys only while key input is off. Tests run with `XFB_NO_INPUT=1` from a test preload (`bunfig.toml`), which makes the sender refuse | [offline] gate, phase refusals, the re-check, fake sender, binding refusals, key record layout; in game [unverified] |
...
| Opening the creator gated | `cc.open` needs the same switch, refuses outside normal play, in combat, a vehicle or a scene, where photo mode isn't allowed or while the game locks saving for combat, a scene, a tier or a moving platform, and asks the menu only after the game reports saving locked. The request is inert after three seconds, withdrawn on a timeout and by the kill switch; a screen already open stays for the player to leave (Back discards) | [offline] unit checks and the self-test host; in game [unverified] |
| Render options read through CET | `game.options.read` is read-only on both sides: the CET layer only calls `GameOptions.Get` and answers once per request; the plugin accepts an answer only for the pending request, only the names asked for, values as text up to 256 characters, 64 KiB at most; the kill switch releases a waiter and refuses later answers | [offline] unit checks and the self-test host's simulated CET layer |
...
1. Are script calls from the RED4ext `Running` `OnUpdate`, made with a context (§3.5), safe during loading screens? Main menu, gameplay and photo mode: yes, with a context [runtime] ([test card](runtime-bridge-test-card.md)).
