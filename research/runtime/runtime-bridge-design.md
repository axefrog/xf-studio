...
| `photo.enter` | write | bridge, then game | Codeware `QuestsSystem.ExecuteNode` with a `questUIManagerNodeDefinition` holding `questOpenPhotoMode_NodeType` (`alwaysAllowTPP`) | [source]; working outside quests is a [hypothesis] |
| `photo.exit` | write | game | `gameuiPhotoModeMenuController.OnExitConfirmed(true)`, the menu's own exit (`photoModeMenuController.script:140, 218`); refused while `IsExitLocked` | [source] |
| `photo.camera.set`, `photo.light.set`, `photo.expression.set` | write | game | `GetMenuItem(key)`, then `PhotoModeMenuListItem.ForceValue(value, true)`, the menu's own path to `OnAttributeUpdated`. Values are checked against the captured range or option data first, because `ForceValue` silently selects option 0 for an unknown option value. Lights: select (key 43), wait 150 ms, then set | [source]; keys below |
| `photo.hud.hide` | write | game | Our `@addMethod` on the controller around its protected `OnFadeVisibility(Float)` | [source] |
| `cc.apply` | write | game | `ApplyChangeToOption`, exactly as the mirror screen's controls (`characterCreationBodyMorphMenu.script:481, 814, 828`); only while that screen edits the finalized look (`m_updatingFinalizedState`, a live `GetPuppetPreviewGameController`) and is not busy switching appearance (`m_busySwitchingAppearance == AVAILABLE`); option by internal name or on-screen label, index range-checked. It never confirms (`ReFinalizeState` is never called); Back (`CancelFinalizedStateUpdate`) discards | [source] |
| `world.time.set` | write | game | `TimeSystem.SetGameTimeByHMS`; exact restore with `SetGameTimeBySeconds` | [source] |
| `world.pause` | write | game | `SetTimeDilation(reason, 0.0)` and `SetTimeDilationOnLocalPlayerZero`, the way the mirror screen freezes the world (`characterCreationBodyMorphMenu.script:1107-1117`), under our own reason. `SetPausedState` takes **no** parameters in the 2.31 bundle (the pre-2.3 dump lists two), so it is not used | [source] |
...
| No saves, and a save lock | Nothing on the allowlist saves. Before its first change the bridge asks `SaveLocksManager.RequestSaveLockAdd` for a lock (`core/systems/saveLocksManager.script:30-48`, the request vanilla uses for autodrive), which is not persistent and is gone after a load; the kill switch releases it | [source]; blocking autosaves in practice [unverified] |
| Reversible writes | Each write returns `undo {method, params}` and logs it; the tools add a plain undo note to every write command and an audit line before sending | [offline] |
| Restore on kill | After the kill switch, the next Running tick runs `RestoreAfterKill` once: unfreeze the world, show the photo-mode menu, release the save lock | [offline] in the self-test host; [unverified] in game |
