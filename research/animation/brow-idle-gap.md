...
The next source check named above is now done offline ([facial expressions §5](../../knowledge/facial-expressions.md#5-the-character-creator-idle), [evidence](expressions-evidence.md)). The creator's face graph `player_woman_paperdoll_sermo.animgraph` loops `ui_closeup_shot` by default and, when `AnimFeature_Paperdoll.characterCreation_Eyes` is set, plays `ui_closeup_shot_eyes` once before returning to the loop; the preview controller sets that flag for the `UI_Eyes` camera slot, i.e. while the creator's eyes section is open [resource] [source]. The supplied close-ups with strong brow movement are therefore consistent with the eyes showcase clip rather than with the loop the Studio plays. This still needs a matched in-game capture to confirm; no change to the Studio's default idle follows from it, and the showcase clip must not be substituted for the idle.

The same study found that every vanilla player `face_rig` component, female included, references the male player facial setup `h0_001_ma_c__player_rigsetup.facialsetup`, whose pose data differ from the female basehead setup the bake uses [resource]. Whether the engine solves with it is a runtime question in the [expressions brief](../backlog/expressions-and-idles-brief.md#runtime-questions-for-one-batched-session) (R1); if it does, the bake's setup choice is a second candidate for magnitude differences.
