...
## 11. Decisions (27 September 2026)

| # | Question | Decision |
|---|---|---|
| Q1 | AMM-only custom poses | **Deferred.** AMM is very popular, so its pose format is kept in mind as a possible opt-in "custom support" feature later, if people ask for it. Keep the pose source abstraction open to a second provider. |
| Q2 | Browsing and previews | **An expandable tree** (category → poses) where one click applies a pose; no stepping through the list. No previews in the first version; later, an **SVG outline preview** generated by analysing each pose (a silhouette or stick figure from the decoded joints), shown in the tree, so a pose can be seen before it is chosen. |
| Q3 | Camera when a pose is chosen | The camera stays **fully under the user's control**; choosing a pose just doesn't re-frame it automatically. "Frame V" and the whole-body view frame the posed body on request. |
| Q4 | Outfit rules | **Do what the game does** by default: the game's filters apply as the game applies them. An explicit "unsafe" option may deviate later where the game restricts something. |
| Q5 | Face with a pose | **As photo mode allows**: poses and expressions combine freely. With the view graph (design in progress), each view chooses what feeds its V: pose, expression, idle, independently per view. |
| Q6 | Favourites per user | **Yes**, plus Recent. |
| Q7 | Weapons and props | **Badge first**; loading and drawing the prop the game attaches for a pose is a later, researched phase, not ruled out. |
| Q8 | Other body gender's poses | **No.** |
| Q9 | Pose in looks or exports | **No**, Studio view state. (The game's three photo-mode save slots are a separate topic.) |
| Q10 | Helper-joint deformation graph | **Consistent with the game in the end**; phase it after the first release for ease of development. |
