...
**Delivered and merged into `main` on 24 September** (merge `95516b2`; portrait-head follow-up `038054e`). The Opus 5.5 redesign replaced the fixed sidebars with a dockable interface. Production entry: `public/index.html` → `src/studio-main.ts` (trusted composition root) → `src/studio-ui/` (presentation, mounted with only `StudioPresentationPort`). The legacy shell `src/main.ts` stays at `/legacy.html` until Nathan accepts the new UI in depth. Delivery record, audit, evidence and acceptance: [ui-overhaul-2026-09-24.md](../authoring/ui-overhaul-2026-09-24.md). Current boundary state: [ui-architecture-boundary.md](../authoring/ui-architecture-boundary.md).

Nathan has reviewed it only cursorily ("seems very solid").
...
1. **Nathan's in-depth review** of the new UI; then retire `legacy.html` / `main.ts` once he accepts it.
...
3. **API gaps** from the audit: A-1 (dirty state), A-5 (geometry/navigation commands; blocks arrow-key nudging and scripted coordinate edits), A-7 (`limitsFor`), A-8 (structured validation issues), A-9 (request IDs/honest cancel), A-10 (Undo label/Redo), A-12 (per-layer readiness), A-13 (selection visibility in viewport snapshot), A-14 (disclosure in UI prefs; revision bump on content only). Partly open: A-2 (consequence metadata), A-3 (unified activity model), A-4, A-6, A-11, A-15. Signatures are in the delivery record's appendix.
4. **Move file-workflow IDs** (`StudioFileOperations`) into the `StudioApplication` static descriptor registry.
...
6. Adapter/core notes still open: case-sensitive Ctrl+Z (`key === "z"`) gesture cancel in `uv-editor.ts`/`surface-editor.ts` (B-7), Shift overload on the head (B-17), control transactions vs gestures (`AuthoringControlEdits.cancel`), `layer.edit add` should refuse without a selected preset, and `CollectionService.capability()` re-validating the whole workspace including Undo histories (~27 ms).
7. Presentation follow-ups: arrow-key nudging (after A-5), UV units (C-17), stable warp names (C-16), virtualised lists for large collections, screen-reader verification, shorter stage hint in narrow head cells.

## Requirements that remain binding for future UI work

These came from Nathan's brief and still apply to any change to the Studio UI.
