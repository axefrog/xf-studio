...
| Matched lighting for colour calibration | **Specified, awaiting implementation and capture.** The creator and the mirror share one render-to-texture scene: a black box, 15 spot lights (female), a 15° camera, fixed exposure and a LogC3 grading LUT (Nova LUT 3.0 wins on the reference install). The page gives a Three.js preset spec and a numeric capture protocol that also carries the hair-colour ladder. [Creator lighting](../../knowledge/creator-lighting.md), [evidence](../character-customization/creator-lighting-evidence.md) |
| Render **all** CC details | **Open.** Currently: head (base-game default skin, not the saved `skin_type_05` chain), expanded plate, eyes (saved Kala diffuse), brows, lashes and hair (resolved for any V since P0; [head CC rendering](../../knowledge/head-cc-rendering.md#6-render-plan-ranked-by-visual-gain-per-effort)), vanilla/PRC piercings. Missing or approximate: saved skin material, teeth/mouth parts, cyberware/skin overlays, other CC categories. Depends on [materials RE](materials-shader-re.md) and [CC controls](cc-controls-and-presets.md). |
