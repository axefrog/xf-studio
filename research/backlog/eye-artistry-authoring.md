...
| Not urgent; explicit request | Editable layer and preset collections | Add/remove makeup layers and drag to reorder. Above the layers, an accordion list of presets with add/remove, rename and reorder; NO preset dropdown. Editable layers are now implemented (0–32 preview budget), with stable IDs, add/copy/rename/remove, pointer reordering, up/down controls, empty stacks and Undo. Recipe-1 imports remain supported. The accordion preset list and preset CRUD/order are still queued; the dropdown library remains temporary. Preserve stable IDs, selection, undo/recovery and explicit order through saving and export. |
