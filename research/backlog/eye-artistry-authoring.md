# XF Appearance Studio authoring requests

Scope expanded by Nathan on 2026-09-23: this is now XF Appearance Studio, with other appearance details and potentially full-body editing later. Continue the first makeup milestone while designing reusable surface, asset-resolution and material tools. All new archive appearance names use `xfas_`, including expanded mesh appearances; see the [naming contract](../../projects/xf-appearance-studio/data/naming.md).
...
|---|---|---|
| Explicit requests; no assigned priority | Modded eyes, adjustable FOV, all-state persistence, SSS/material fidelity, imported-V reload bug | [Detailed requirements and screenshot references](preview-fidelity.md). Earlier session-only imported-V behaviour is superseded; resolve modded eye winners and diagnose eye/lip highlights rather than treating SSS as a universal fix. |
...
| First slice complete | Optional head details, especially brows, to judge makeup against | Saved Arkhe brows 18 and Soft Natural lashes render with independent toggles, morphs and exploratory blink. Colour/material approximation is labelled. Skin/eye overlays, hair and other details remain. |
...
| Not urgent; explicit request | Editable layer and preset collections | Add/remove makeup layers and drag to reorder. Above the layers, an accordion list of presets with add/remove, rename and reorder; NO preset dropdown. Current four-layer schema and dropdown library are temporary and do not satisfy this request. Preserve stable IDs, selection, undo/recovery and explicit order through saving and export. |
