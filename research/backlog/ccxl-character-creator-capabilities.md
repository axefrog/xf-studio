...
Queued explicit request (23 September), **track 8** in the [ranked queue](README.md). Keep it; pull findings forward when they unblock [CC controls and presets](cc-controls-and-presets.md) (track 3) or the [CC file-chain study](cc-file-chain.md) (track 5).
...
6. **Prototype the most useful discoveries.** The current XF Studio priority is ONE selector switching complete authored presets: atomically select all components/materials, handle Off, and preserve stable save identities across collection updates. Independent design/colour/finish selectors are no longer its product target. Broader research into linked/conditional controls, grouping/paging, extra morphs and richer previews remains useful, but later feature implementations require discussion with the maintainer first.
