...
- **Built:** the diagnostic route with the window material, the nested flake chains and the emissive accent chunk, and the six-preset board ([experiment 021](../../experiments/021-glitter-board/README.md)). **Still needed from the Studio:** a game-matched Glitter optics model with physical fields that authors those flakes (and the preview that follows the route's maps), before any Glitter layer can export without the diagnostic knob.

### Open

1. Use the [finish board](../../experiments/016-finish-board/README.md) and [experiment 017](../../experiments/017-plate-depth/README.md) sessions to tune or withdraw the experimental Glossy, Shimmer and Colour-shifting adapters. Glitter's next candidate is the resolved glint-flake route ([design](../materials/finish-designs/glitter.md)). Its diagnostic board is built and verified offline ([experiment 021](../../experiments/021-glitter-board/README.md)): the V sign on its decoded maps, BC5 on 3-texel flakes and WolvenKit keeping the supplied nested chains all check out. Next: stage it after session 2 and run its test card.
...
5. Test redacted-c01's full conceptual sequence (below) in an independently authored study.
