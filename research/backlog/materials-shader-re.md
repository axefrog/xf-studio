...
- **Ranked follow-ups** (the decal and multilayered items are argued in [decal §10](../materials/shader-decal.md#10-recommended-changes-ranked)):
  1. **Done (27 September):** the glitter board's accent writes `EmissiveEV` 1 and its test card carries the modifier check.
  2. **Done (27 September):** Colour-shifting metalness 0.25 → 0.08, so the tint keeps the skin's SSS.
  ...
  5. *Export:* Matte roughness 0.88 → 1.0 is **done (27 September)**; after the *Gloss D* verdict, partial surface weight or grain for flat finishes if they still read "plastic".
  ...
  11. *Preview (metal and glass):* ranks 1–4 of [metal and glass §7](../materials/shader-metal-glass.md#7-recommended-preview-adapters-ranked): a `metal_base` adapter (R-channel metalness and roughness, alpha test), an environment for the creator preset, and a two-pass `glass_onesided` adapter (transmittance, then a sun-and-probe reflection with no local lights). Together with the holster-state rule ([body rendering §1](../../knowledge/body-rendering.md#1-which-parts-make-the-third-person-body)) they unblock arm cyberware. **The creator environment is now specified** ([creator lighting §10–11](../../knowledge/creator-lighting.md#10-indirect-light-what-reaches-v)). The box has no probe, GI or bounce, so the preset keeps no environment by default and gains an optional uniform ambient cube that the piercing check fits. It is not a structured environment.
