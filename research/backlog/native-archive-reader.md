...
**One production read** (`claude/clothing-render`): the clothing host decodes the game's cooked appearance-name visual-tag preset (`base|ep1\entities\appearancename_visualtags.json`, a `JsonResource`), which WolvenKit 9.0.1 doesn't serialize, through `NativeArchive` and `readResourceJson`, from the archive that wins its path, and caches a compact table per archive identity (`clothing-host.ts`; [worn clothing §4.1](../../knowledge/clothing.md#41-where-visual-tags-come-from-source-wiki)). Nothing else in the app reads through the native reader.
