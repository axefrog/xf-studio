...
| 2 | **Character detail rendering completeness** — brow, lash and hair *colours* correct first (a long-standing, top feature priority), then render **all** character-customisation details in the viewport | Open. Brow material path corrected from source (study approximation); lash colour source ambiguous (Alliekat vs vanilla `brown_liquorice.hp`); hair colour uses an approximate dark brown instead of the `ash_brown.hp` profile. Skin/eye/lip parity not reached. | [Head CC rendering plan](../../knowledge/head-cc-rendering.md) (ranked: resolver-fed render record, skin material, one face-decal path, creator controls, full eye, general hair), [preview fidelity](preview-fidelity.md), [brow/lash study](../eye-artistry/brow-lash-fidelity.md), [hair profile resolution](../eye-artistry/saved-hair-profile-resolution.md) |
...
- **README presentation (low priority, after the alpha wording lands):** a theme-aware title banner (the XF mark and wordmark as light and dark SVGs via `<picture>`), and a pair of light- and dark-mode Studio screenshots shown as modest thumbnails linking to full-size images. Default: real app screenshots including the game-rendered head, with a "Cyberpunk 2077 © CD PROJEKT RED" credit line.
- **Guidance system: tours, spotlights and a Help view** (after the alpha; lower priority): data-driven onboarding, on-demand how-to and per-release "what's new" tours over stable UI anchors, a theme-aware spotlight overlay, configurable callouts, and a searchable Help view that launches tours. Designed as a platform capability; see [feature-module platform §6a](../authoring/feature-module-platform.md#6a-guidance-tours-spotlights-and-help-planned).
