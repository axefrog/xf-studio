...
- **Python-free Build (next after the built-in plate lands):** the product Build path still runs about 800 lines of experiment-005 Python (`build.py`, `mip_maps.py`, `verify.py`, `archive_inventory.py`, `tools/build_collection_package.py`) and requires users to supply Python with NumPy and Pillow. Port it to a TypeScript compiler module in the app, keep the verifier as an independent code path, and keep Python for research only. The end state is that Build needs only the game folder and WolvenKit.
...
- **Framework handling in tooling (next, small):** remove framework installation from the runtime-diagnostic stage/promotion tools and replace it with a read-only framework version check. It reports installed ArchiveXL/TweakXL/Codeware/RED4ext/redscript/CET versions and friendly update guidance. Staging adds only XF Eye Artistry, placed within the user's MO2 separator sections. After the first in-game test, help the maintainer update frameworks normally and remove the diagnostic profile and the three "XF Studio Framework" copies.
