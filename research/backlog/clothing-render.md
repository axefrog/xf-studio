...
| 3 | **Hiding and body context**: tags, the vanilla slot table, `.xl` tag rules, `partsOverrides` onto the body, `{body}`/`{feet}`/`{arms}`/`{sleeves}` | M | **Built**, except `{body}`/`{arms}`/`{sleeves}` substitution, which only dynamic items use (phase 5); the feet group follows footwear |
