...
A faithful eye cannot be obtained by swapping diffuse alone. The actual material includes `NormalBubble`, iris/refraction/parallax controls and `RoughnessScale` about 0.493421; its reflection cubemap (`Blick`) and `SubsurfaceFactor` are read by no 2.31 program ([eye shader reference](../materials/shader-eye.md#22-parameters)). These are shader-specific parameters, not directly interchangeable with Three.js controls. Export and inspect the normal/roughness maps, inspect `eye.mt` and its shader behavior, and check any active Character Rendering Editor runtime changes before claiming parity. This supplies a concrete input for the already queued waxy-eye/fidelity investigation without introducing another feature editor.
