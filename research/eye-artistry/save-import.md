...
The long visible hair is another reason not to interpret the save's `Short` tag as a reliable description of final modded geometry. Resolve the saved hair resource and installed overrides. Do not assume the photos correspond exactly to AutoSave-12's moment or infer preset IDs from their pixels. Start with resolved resources, then compare a similar camera/pose and lighting; reserve any fresh game captures for one batched fidelity check.
...
Current implementation: [reader](../../projects/xf-appearance-studio/authoring/src/save-reader.ts), [CLI](../../projects/xf-appearance-studio/authoring/tools/read-save.ts). Browser file selection is local; save bytes are never posted to the server. The CLI operates on the captured copy. Tested end-to-end on this 2.31 save; the supported version guard is not a claim of exhaustive compatibility with all older saves.
...
3. Assemble the selected skin, eye overlay, eyes, teeth, brows, lashes, hair, decals and rig. Handle component visibility and perspective groups deliberately. Saved clothing/equipment and script-driven appearance changes may need additional nodes or a runtime snapshot.
