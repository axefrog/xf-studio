...
3. Complete the selected skin, eye overlay, teeth, decals and rig, and calibrate the now locally resolved brows, lashes and hair. Handle component visibility and perspective groups deliberately. Saved clothing is read: the vanilla equipment system's loadout, wardrobe visuals and hide flags from `ScriptableSystemsContainer`, and the active wardrobe set from `WardrobeSystem_ClothingSets` (`save-loadout.ts`; [worn clothing](../../knowledge/clothing.md#26-what-the-studio-reads-source)). Script-driven appearance changes (an outfit mod's outfit) need a runtime snapshot.
