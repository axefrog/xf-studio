...
1. **For a feminine V the Studio resolves and draws every creator row a player sees on a typical character.** Skin type and tone, all five face-shape regions, eyes, lashes, brows, hair (including the face-cyberware hairstyle twin), every face decal (eye makeup, lipstick in all three finishes, cheeks and freckles, blemishes, scars, facial tattoos, face cyberware), piercings, the teeth, the body, arms, hands, nails (colour and length), flat feet, breast size, body tattoos and body scars all go through the generic resolver and a material adapter chosen by the template's own name [source].
2. **The teeth were the one feminine head row that resolved but was never drawn** (rank 1, closed 27 September). They now have a slot of their own, drawn by their chunk's template: the default through the skin adapter with the teeth's own skin profile, the finishes through the layered adapter ([head CC rendering](../../knowledge/head-cc-rendering.md#1-every-head-option-at-a-glance)). That also meets the subsurface-scattering gate's precondition that the mouth interior is present ([ownership gate](../../projects/xf-studio/authoring/evidence/render-fidelity-ownership-gate-2026-09-25.md)).
...
4. **The biggest fidelity gaps on drawn parts are lighting, not selection:** the subsurface stand-in on all skin (decoded down to the kernel, not ported), and hair physics (none). The hair profile bake and the ambient hair path were ported on 27 September (rank 2).
...
| **Teeth** | 5 (default, silver, gold, copper (spelled `cooper` in the resources), pink) | `ht_000__basehead.app` → `ht_000_pwa__basehead.ent` → `ht_000_pwa__morphs.morphtarget` (21 `mouth` targets) → `ht_000_pwa_c__basehead.mesh`: **one chunk, 3,210 vertices**, drawn in scene and shadows; 8 mesh appearances (the creator reaches `teeth_001` and the four finishes `teeth_007__silver`, `teeth_003__gold`, `teeth_002__cooper`, `teeth_006__pink`; `teeth_004__metal`, `teeth_005__metal_rusty` and `default`, whose material is `metal_base.remt`, are unreached). `teeth_001` → `teeth_base.mi` → `skin.mt` with `customisation_teeth.sp`; the four finishes are `multilayered.mt` (`ht_000_base__<finish>.mlsetup`, the shared gold `.mlmask`) [resource] | **Drawn** (27 September) in the `teeth` slot: the default through the skin adapter, the finishes through the layered adapter; follows the `mouth` shapes and the idle [source] | Mouth interior lit without occlusion (bright) [hypothesis] | – |
...
|---|---|---|---|---|---|
| 1 | **Done (27 September).** **Teeth and mouth interior**: the `teeth` slot on the creator slot `teeth` (groups `TPP`), drawn by its chunk's template: the skin adapter with the chunk's own skin profile (`customisation_teeth.sp`) for the default, the layered adapter for the four finishes (all `multilayered.mt` [resource]) | Every V, whenever the lips part (idle, blink frames, later expressions and photo-mode faces) | **S** | – | The mod that lists the teeth component 16 times draws it once (first drawable per name; its 15 hash-only copies carry LOD mask 0 anyway [resource]); the teeth are never the skin under a lip decal; the unreached `default` appearance (`metal_base.remt`) is said plainly until rank 5's adapter |
| 2 | **Done (27 September).** **Hair colour and ambient light as the game bakes them**: the `.hp` bake (sort, rescale to 0–1, sample at `k/N`, truncate to bytes) and the decoded ambient hair path ([hair reference §11.1](../materials/shader-hair.md#111-recommended-preview-changes-for-a-code-track)) | Every V with hair or lashes (brows use a decal gradient, not a `.hp`) | **S** (bake), **S–M** (ambient) | – | Changes every hair colour at once; the swatch comparison with the bake gives a median 4.3° hue error over 24 vanilla swatches (4.8° for the earlier model) |
...
| **Teeth** | Creator teeth page (`UI_Teeth` camera), each of the five choices under one light, and one photo-mode smile: colour, the metal choices' highlight, and whether the gums and tongue read inside the mouth | Head CC 15 |
...
- **Teeth** facts were added to head CC rendering §1 (mesh and appearances), and corrected when rank 1 landed: the four creator finishes are `multilayered.mt`, not unread `.mi` templates.
