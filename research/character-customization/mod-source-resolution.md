...
A save stores appearance resource hashes and definition names, not the bytes that rendered them. The [saved-V reader](../../projects/xf-studio/authoring/src/save-reader.ts) preserves unsigned 64-bit hashes as decimal strings. The preview's eyes now come from the generic resolver like the other character details ([eye rendering §6.6](../../knowledge/eye-rendering.md#66-implementation-status)); the earlier one-pair eye manifest is retired. In the [reference save's eye trace](../eye-artistry/modded-eye-resolution.md), `eye_16_diffuse` is absent from the serialized shared `.app`, registered by nutboy's CCXL resource and mesh patch, and ultimately uses Kala's texture archive. A search of the base `.app` alone would reject a valid saved choice.
...
At this inspection, the reference MO2 instance's `ModOrganizer.ini` selects profile `2025 (again)` and sets `enforce_archive_load_order=false`, `reverse_archive_load_order=false`. That profile enables Unique Eyes to CCXL and Kala's Eyes Standalone V2. No `<MO2>/overwrite/archive/pc/mod/modlist.txt` exists. The local installed Cyberpunk support plugin (`F:/Games/MO2/plugins/basic_games/games/game_cyberpunk2077.py`, version 2.3.1, authors listed as 6788 and Zash) reads active mods by profile priority and generates an archive `modlist.txt` in its launch hook only when enforcement is enabled. The [plugin's maintained source](https://github.com/ModOrganizer2/modorganizer-basic_games/blob/3bd9da97c159a1bc05fd21e81199e997ed16e0f6/games/game_cyberpunk2077.py) shows the same separation; the [maintainers' Cyberpunk load-order guide](https://github.com/ModOrganizer2/modorganizer-basic_games/wiki/Game%3A-Cyberpunk-2077#load-order) says the default game archive conflict winner is the first archive in alphabetical load order. When enforced, first in the generated list wins: `reverse_archive_load_order=false` puts lowest MO priority first, while `true` puts highest priority first. Direct file conflicts still follow MO priority, with `overwrite` highest. These are rules and current configuration, not evidence of the launch path used for Nathan's save or photographs.

For **base-game versus mod archive hash collisions**, keep that last distinction explicit. [WolvenKit's pinned offline resolver](https://github.com/WolvenKit/WolvenKit/blob/11720772f1e20581301b3dec88a59f7b5ee05675/WolvenKit.Modkit/Managers/ArchiveManager.cs#L587-L628) searches mod before EP1/base; [ArchiveXL 1.26.3](https://github.com/psiberx/cp2077-archive-xl/blob/d4beb964ce61de722e083f1fb976bc31e4cb9f49/src/App/Archives/ArchiveService.cpp#L95-L120) inserts extra mod groups ahead of non-mod groups. Neither source implements or displays REDengine's native collision lookup for the photographed session, and MO2's enabled state does not prove the launch's mounted set. The concrete two-payload [saved-lash collision](../eye-artistry/brown-liquorice-profile-overlap.md#loader-source-boundary-25-september-2026) remains unresolved in the catalog until a version-matched native lookup/mount trace or resource-request observation supplies that missing evidence. Do not turn tool-side precedence into a general engine rule.
