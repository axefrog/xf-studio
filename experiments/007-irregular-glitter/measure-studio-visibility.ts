...
import {initialRecipe,coverage,raster} from "../../projects/xf-appearance-studio/authoring/src/recipe";
import {createRegionFlakeCatalogueJob,createFlakeBakeJob,defaultStudioIrregularFlakes,
  STUDIO_FINE_REGIONS} from "../../projects/xf-appearance-studio/authoring/src/flake-field";
...
function run(count:number,radius:number){
  ...
  const catalogueJob=createRegionFlakeCatalogueJob(settings,STUDIO_FINE_REGIONS);
  ...
  const maskCentres=catalogue.flakes.filter(f=>coverage(f.u,f.v,layer)>0).length;
  const diameters=catalogue.flakes.map(f=>f.radius*2).sort((a,b)=>a-b);
  const sizes=[1024,2048].map(size=>{
    const mask=raster(layer,size),job=createFlakeBakeJob(catalogue,size,4,"covered-average");
