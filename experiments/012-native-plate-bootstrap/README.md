...
XF Studio now ships this experiment's triangle selection as an asset-free recipe, [`eye-plate-recipe.json`](../../projects/xf-studio/authoring/src/eye-plate-recipe.json). It holds the 105 face ranges, the face/vertex ID hashes (`555c2f7e…` / `5c37e347…`), the expected topology and the SHA-256 of the audited 2.31 head mesh and morph resources (`e877b91a…` / `3e10c3f7…`, as extracted above). Build extracts the two resources the game is expected to load for the user's launch route (the base-game copies from `archive/pc/content`, or a mod's copy or `.xl` patch when one wins and passes the recipe's topology gates) with WolvenKit, cuts them in TypeScript directly over WolvenKit's JSON form, converts the result back and independently re-verifies it against the head actually used. On the reference installation's MO2 profiles the morph target comes from the Facial Customisation Rig Fix, whose geometry equals the base game's, so the plate mesh is unchanged and the plate morph carries the fix's renamed target bone names. The derivation, cache and readiness are described in the [pipeline guide](../../research/authoring/studio-to-mod-pipeline.md#where-the-eye-plate-comes-from).
