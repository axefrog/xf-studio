...
The candidate uses the exact sampled-head positions and affine skin matrices, then checks all **73** preserved idle poses against every head triangle. Separately, it evaluates Basis, each of the **105** morphs and the reference save's five-morph combination: **107 static cases**. Every static case is checked for total displacement, neighbor smoothness and whole-plate/head triangle contacts. [The report](finite_correction_summary.json) contains the case-level counts, input hashes, seed choices and gate results.
