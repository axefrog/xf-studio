...
| `memory_guard.py` | Runs a command inside a Windows Job Object and kills the whole process tree when its private memory passes `--limit` GB (default 8) or the machine's free memory falls below `--floor` GB (default 16); a hard commit cap at 1.25× the limit backs up the polling, and the tree dies with the guard. A kill exits 99 with a plain message and is appended to the ignored `local/memory-guard.log`. Wrap every heavy command in it: `python tools/memory_guard.py --limit 6 -- bun test`. Use the full interpreter path when the wrapped process reads the Studio's saved settings (a dev server): the `python` alias runs as a packaged app with redirected AppData, which its child processes inherit. |
