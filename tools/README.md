...
| `inventory/report.py` | Builds Markdown/JSON inventories from the pinned survey and curated `inventory/classifications.json`. Update its snapshot date for a later survey. |
...
All scripts capture paths/metadata rather than credentials. The Nexus credential lives outside HQ under Windows CurrentUser DPAPI, not in a script or config. Reference extractions and large manifests are locally available but excluded from Git/release packaging.
