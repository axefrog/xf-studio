...
| Eye colour | `he_` mesh appearance → eye `.mi` → textures | Resolved eye component of the shown V (creator slot `eyes_color`) through the eyeball and wetness-shell adapters (`src/eye-material.ts`): colour sampled as the game does, the iris ramp baked from the resolved gradient stops, layered designs through the layered adapter ([eye rendering §6.6](eye-rendering.md#66-implementation-status-ranks-13-built-ranks-47-open)) | **Save** or default V | Creator selector; refraction, cornea and iris normals and the Eye-class light (ranks 4–5) |
