...
### Hair-colour additions

Every CCXL hair-colour pack on the reference installation (six, 150 colours) follows the wiki's hair-profiles template ([study](../research/hair/hair-colour-authoring-feasibility.md)):

- **Anonymous slot overlays** [source + resource]. The pack's creator resources hold only unnamed appearance options with `uiSlot` `hair_color`, `eyebrows_color`, `eyelash_color` (and `beard_color` in the masculine resource), each listing the colours. ArchiveXL merges every mod's named options before any overlay (`MergeCustomEntries`), so the colours reach all 73 feminine and 78 masculine hairstyle colour options, cyberware twins and CCXL hairstyles included, whatever the load order. Each overlay appends the same list to every matching option, which keeps the `hairstyle color` link index aligned for styles that start from the 35 vanilla colours. Exact-slot overlays also reach `hair_color_none` (the bald choice) and feminine `eyebrows_color0` (brows Off); what those rows then show is untested.
- **Definition name → mesh appearance** [source]. `FixCustomizationAppearance` strips a leading `NN_` (two digits and an underscore), takes what follows the last `__`, or else uses the name whole (`38_ash_brown` → `ash_brown`; `sigh4_mch_blondeice` unchanged). A pack's `patch.mesh`, patched into the player hair, hat-hair, brow, lash and beard scopes, adds that empty appearance with ten `@family` templates whose paths expand `{material}` to the pack's `.hp` and cap gradient ([mod loading §5](mod-loading.md#5-archivexl-dynamic-materials-mesh-side)). ArchiveXL's own fix gives 295 vanilla meshes the matching template families.
- **Tags** [source + wiki]. For `hair_color` targets the choice's `tags` go to TweakDB `ItemFactory.HairColors.hairColors`; garment dynamic appearances substitute the chosen colour's tag as `{hair_color}`, so hair under hats needs the tag to equal the mesh appearance name. One installed pack leaves all 21 tags at the template placeholder.
- **Brows, lashes and beard** get the same names in their own rows, with no engine link to the hair row. Brows draw the cap gradient's last texel; lashes draw the profile's tip ([eyebrows §3](brows.md#3-material-and-colour), [hair reference §9](../research/materials/shader-hair.md#9-lashes)).
- **Rendering never consults the creator list**: a saved `(app, definition)` is cloned from the requested name alone, so a colour whose definition is gone but whose patch appearance and files remain should still render [hypothesis; the study's checklist item 9].
...
1. **Installation view:** base game `archive/pc/content`, `archive/pc/ep1` (if present), `archive/pc/mod`, the ArchiveXL bundle directory, plus the mod manager's virtual view (MO2 profile with overwrite, or manual files). Vortex needs no adapter for the files, since it hard-links them into the game folder ([Vortex §5](vortex.md#5-what-xf-studio-sees)); REDmod is a later adapter. [Source discovery](../research/authoring/source-discovery-foundation.md) enumerates the files; `resolver-host.ts` reads RDAR indexes and reads resources natively first ([archive formats](archive-format.md)), falling back to WolvenKit CLI per resource with an ignored JSON cache ([mod loading §6](mod-loading.md#6-implementation-and-reproduction)).
...
| R12 | **Full body:** visual-tag hiding, garment offsets, feet/genital controllers. | ArchiveXL `VisualTags.xl` | **Partly implemented** for worn clothing: `.xl` tag rules and entity-wide `partsOverrides` onto the body and arms, and the feet group from footwear ([worn clothing §6](clothing.md#6-how-the-studio-draws-worn-clothing)); garment offsets and garment support deferred |
...
Source: audit of `projects/xf-studio/authoring/src` on 2026-09-25, rows updated as later steps landed. Since 26 September the Character panel (`src/cc-panel.ts`) lists every creator option, so any detail the preview draws can be changed there as well as read from a save. The per-option templates each gap needs and the ranked render plan are in [head CC rendering](head-cc-rendering.md#6-render-plan-ranked-by-visual-gain-per-effort). "Selectable" means a Studio control exists; "Save" means it renders only when an imported save matches.

| Detail | Needed to render | Path today | Status | Gap |
|---|---|---|---|---|
| Head mesh + facial morphs | head `.morphtarget`/`.mesh`, 105 targets | Derived from the user's installed game with all 105 targets and the eye plate cut from the same rows (`src/preview-core-*.ts` over `src/game-asset-export.ts`), loaded through one typed render record (`src/core-detail-loader.ts`); this is the only source of the preview head | **Save** (5 regions) + **selectable eye shape** (22 choices, listed from the head's `eyes` targets; the derived eyes follow it) | Nose, mouth, jaw and ear shapes selectable in the Character panel; male head absent; vanilla base-game head only, not the effective mod winner |
| Skin tone / type | 5 type `.app`s × 12 tones → `skin.mt` chain (albedo, packed normal, roughness R/B, microdetail, skin profile) | Resolved skin chain of the shown V through the skin adapter (`src/skin-material.ts`, P1: winning albedo including complexion mods, tint, detail and microdetail, secondary albedo, skin profile) ([head CC rendering](head-cc-rendering.md#6-render-plan-ranked-by-visual-gain-per-effort)) | **Save**, default V or any creator choice | Screen-space SSS not ported (a per-channel wrap stands in) |
...
| Eyebrows + colour | one `heb_` morph/mesh for every style, a texture set per style, decal material + gradient ([eyebrows](brows.md)) | Resolved component of the shown V through the double-diffuse adapter (`src/character-detail-*.ts`, `src/brow-material.ts`) | **Save** or default V + visibility | The brow's normal and roughness writes; colour accuracy unverified in game |
| Eyelashes + colour | vanilla: `he_` chunk 0 + `eyelashes__<colour>`; CCXL: own mesh | Resolved component with its `.hp` through the `hair.mt` adapter | **Save** or default V + visibility | Colour accuracy unverified in game |
| Hair + colour | hair `.app` → mesh(es) + `.hp` + cap | Resolved components (strands and cap, highest LOD) through the `hair.mt` and gradient-recolour adapters (`src/hair-shading.ts`) | **Save** or default V + visibility | Physics; shadow-only parts are skipped by render mask |
...
| Vanilla eye/lip/cheek makeup, pimples | `hx_` morphtarget per style + decal `.mi` | Resolved face-detail components through the decal family (`src/face-decal-material.ts`; [head CC rendering rank 2](head-cc-rendering.md#6-render-plan-ranked-by-visual-gain-per-effort)); CCXL makeup options the same way | **Save** or default V | Draw order within a priority; in-game comparison |
| Facial tattoos, face scars, face cyberware | `hx_` morphtargets, chunk masks, tone link | Same path (tone from the saved definition; scars' and cyberware's normal writes included) | **Save** or default V | Decal templates other than the three drawn are reported, not drawn |
...
| Teeth | `ht_` morph/mesh; `teeth_base.mi` (`skin.mt`) or a finish's `multilayered.mt` | Resolved component of the shown V (creator slot `teeth`) through its template's adapter: skin with the teeth's own profile, or layered ([head CC rendering §1](head-cc-rendering.md#1-every-head-option-at-a-glance)) | **Save** or default V, with any creator choice set | No mouth occlusion; not compared with the game |
...
11. How does the engine treat several components with the same name in one appearance (the reference installation's teeth `.app` from a morph-additions mod has 16)? The preview draws the first drawable one [hypothesis]; in that mod's case the other 15 carry LOD mask 0 on their only chunk, so the question does not change what is drawn there [resource].
