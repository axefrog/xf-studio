...
| Mod texts | ArchiveXL `localization.onscreens` in any `.xl` (a map of language to a path or list; the first language is the fallback; a bare path or list means English; `extend` appends to another file's unit). Per unit in load order: the player's language replaces existing keys, the fallback only fills missing ones; an entry without a primary key is keyed by its secondary key (`Localization/Extension.cpp`). The reference installation declares 924 English files in 735 archives. | [source] |
...
The TweakDB blob is read with WolvenKit's layout (flats by type, `(TweakDBID, index)` keys). A record's field ID is the CRC-32 of the record name continued over `.<field>`, so fields of records known only by ID (the category list's entries) are addressable without their names (`src/tweakdb-flats.ts`).
...
| R11 | **Presentation:** texts, sections, icons. | [Presentation](#presentation-order-rows-sections-labels-and-swatches) | **Implemented** for the game's texts, ArchiveXL text declarations and the compiled TweakDB (`src/cc-catalogue.ts`, `src/game-text.ts`, `src/cc-presentation.ts`) [source + resource]. TweakXL YAML records (mod icons) are not read |
...
| Eyebrows + colour | `heb_` morph/mesh per style, decal material + gradient | Resolved component of the shown V through the double-diffuse adapter (`src/character-detail-*.ts`, `src/brow-material.ts`) | **Save** or default V + visibility | Creator selector for other styles; colour accuracy unverified in game |
