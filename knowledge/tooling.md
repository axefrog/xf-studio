...
- The one runner that applies these rules is [`wolvenkit-cli.ts`](../projects/xf-studio/authoring/src/wolvenkit-cli.ts); consumers map its typed errors. Synchronous callers (the independent package verifier, through the injected [`verifier-wolvenkit.ts`](../projects/xf-studio/authoring/src/verifier-wolvenkit.ts)) use its blocking form `runWolvenKitSync`, which applies the same policy but cannot be cancelled. The character resolver's fetcher and the grading-LUT host use it too: every fetcher on one resolver cache folder runs its batches one at a time in a unique `mkdtemp` batch folder, shares in-flight resources (single-flight), and writes a lasting `.failed` marker (versioned; older markers are ignored and removed) only when `convert` finished cleanly with the extracted file intact and still produced no JSON. Cached JSON is keyed by the WolvenKit identity as well as the archive, and a marker records the identity that failed and counts only for it, so a WolvenKit update extracts again. The archive-index cache is written atomically and trusted only when it holds the entry count the archive's own index declares. The grading-LUT host keeps the opened installation between retries, retries only transient failures (time limit, missing .NET, a process that could not start, a disk error) with doubling backoff and a cap, and treats a WolvenKit exit code on the same archive, entry and WolvenKit as lasting.
...
**The native reader replaces most launches.** XF Studio's own reader decodes archives and CR2W resources through the game's own Oodle library, in a worker. It reproduces WolvenKit's extracted bytes and the resolver's JSON, including resources that WolvenKit refuses to serialize (the mistyped `castShadows` above). The resolver reads through it first and runs WolvenKit only for what it doesn't answer: a cold resolve of the reference save needed no launches instead of 23 ([mod loading §6](mod-loading.md#6-implementation-and-reproduction)). Textures and meshes for the preview still come from WolvenKit's exports. Format and evidence: [archive and resource formats](archive-format.md).
