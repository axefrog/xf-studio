...
- **`{body}`:** body mods declare `player: bodyTypes: [Name]` in `.xl` and patch a component named `Body:Name` (or tag the entity or a morph-target component) into the player entity; otherwise the value is `base_body`, which also covers vanilla-shaped replacers [source: `PuppetState/Extension.cpp` `GetBodyType`; wiki: [bodies], with a table of known body tags]. `GetBodyType` checks the live puppet's entity and visual tags first, then its components from the last, each against every declared body tag (`Body:<name>`, registered in `Configure`) [source: ArchiveXL 1.27.3 `PuppetState/Extension.cpp`]. The wiki's body-mod guide patches a `Body:<name>` `entMeshComponent` into the `player_wa.ent`/`player_ma.ent` scopes [wiki: [bodies]]. Refits then ship `…__{body}.mesh` variants, and ArchiveXL resource links can point several body names at one mesh.
...
3. **Items to garments** (`clothing-resolver.ts`, host ports in `clothing-host.ts`): TweakDB record → factory table (the game's item factories, then every `.xl` factory) → root entity → most specific root appearance for the suffixes the item's own record lists (`appearanceSuffixes`), each evaluated for the V (female/male, third person, `Part` when the shown outer torso item carries `hide_T1part`, the creator's hair tag, the body type, `BaseArms`; `FeetState` and `LegsState` are not evaluated yet) → `.app` definition → components, through the same definition resolver as a creator choice (`resolveAppDefinition`: R6–R10). The body type is ArchiveXL's (`playerBodyType`): `base_body` unless a declared body type's tag is on the third-person player entity (`player_wa_tpp.ent`/`player_ma_tpp.ent`) or an ArchiveXL patch of it; with a type declared but the entity unreadable, or two types present, it stays `base_body` with a gap. Tags are the record's, the cooked preset's, the root entity's and the definition's. The cooked preset is decoded in the native reader's worker (off the host's event loop, after an asynchronous check of the game's Oodle library; the same worker the resolver reads through, asked for this one resource's root class and a longer budget) and kept per archive identity; a failed read is logged and tried again next time. When the TweakDB can't be read, or the clothes can't be worked out at all, the clothing slot is unavailable with one plain line, never "none".
...
6. **Record and drawing** (`xfs/render-detail-10`; the clothing slot came with v9): a `clothing` slot of garment components, lower layer first, each with its `garment` (area, item record ID, layer score: prefix base plus its part entity's size tag, §4.5). Garments draw through the existing adapters (almost all chunks are `multilayered.mt`, baked by the layered adapter; `mesh_decal` chunks as decals; `metal_base.remt` shadow meshes and other unsupported templates are left out and counted). A garment without shape keys of its own carries the body's applied shape (breast size) from the nearest body vertex (`xfs_body_shape`, through one search grid per load), and coincident layers are ordered by a polygon offset of their layer rank; both stand in for garment support (§4.5), which is not built. A garment is kept across loads while the body's shape is unchanged, so a clothing change that only re-masks the body loads and bakes only what changed. The Body toggle hides the clothes with the body, and off, the host prepares neither; the Character panel's **Clothing** section has the setting (only the states that change what V wears are offered), a switch per area the save dresses, and its own Undo and Redo.
