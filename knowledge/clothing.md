...
3. **Items to garments** (`clothing-resolver.ts`, host ports in `clothing-host.ts`): TweakDB record → factory table (the game's item factories, then every `.xl` factory) → root entity → most specific root appearance for the suffixes the item's own record lists (`appearanceSuffixes`), each evaluated for the V (female/male, third person, `Part` when the shown outer torso item carries `hide_T1part`, the creator's hair tag, the body type, `BaseArms`; `FeetState` and `LegsState` are not evaluated yet) → `.app` definition → components, through the same definition resolver as a creator choice (`resolveAppDefinition`: R6–R10). The body type is ArchiveXL's (`playerBodyType`): `base_body` unless a declared body type's tag is on the third-person player entity (`player_wa_tpp.ent`/`player_ma_tpp.ent`) or an ArchiveXL patch of it; with a type declared but the entity unreadable, or two types present, it stays `base_body` with a gap. Tags are the record's, the cooked preset's, the root entity's and the definition's. The cooked preset is decoded in the native reader's worker (off the host's event loop, after an asynchronous check of the game's Oodle library) and kept per archive identity; a failed read is logged and tried again next time. When the TweakDB can't be read, or the clothes can't be worked out at all, the clothing slot is unavailable with one plain line, never "none".
