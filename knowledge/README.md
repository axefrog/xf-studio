...
|---|---|---|---|
| Resource formats and the character-customisation file chain (`.inkcharcustomization` → `.app` → `.ent` → `.mesh` / `.morphtarget` → materials) | `cc-file-chain.md` (to write) | Seed | [File-chain map](../research/character-customization/file-chain-map.md), [CCXL merge boundary](../research/character-customization/ccxl-merge-boundary.md), [catalogue prototype](../research/character-customization/catalog-prototype.md), [mod source resolution](../research/character-customization/mod-source-resolution.md), [head details](../research/eye-artistry/head-details.md), [built-in eye plate](../experiments/012-native-plate-bootstrap/README.md#built-in-production-plate) (byte-level `.mesh` render-blob and `.morphtarget` diff/mapping layout as used by [`eye-plate-cut.ts`](../projects/xf-studio/authoring/src/eye-plate-cut.ts)), legacy xf-omega / `xf-eye-artistry-ccxl` code (reference only) |
