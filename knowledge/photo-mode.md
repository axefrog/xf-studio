...
**A cleaner route for the bridge** [hypothesis]: from redscript, send the same pause event, and in a wrapped `MenuScenario_PauseMenu.OnEnterScenario` call `SwitchToScenario(n"MenuScenario_CharacterCustomizationMirror", data)` with a `MorphMenuUserData` whose `m_updatingFinalizedState` is true. The vanilla `OnEnterScenario` and `OnCCOPuppetReady` then run unmodified.
...
- **The bridge's version** [offline]: `photo.hud.hide` sets a flag that our `@wrapMethod(CursorGameController)` on `ProcessCursorContext` reads (every context becomes `Hide`); the wrap notes each cursor controller it sees and replays each one's own state when the flag changes. Photo mode opening or closing and the kill switch clear the flag (`projects/xf-runtime-bridge/redscript/XFRuntimeBridgeActions.reds`). The next session checks it with the pointer over V's face.
...
| Poses and expressions | In photo mode: attributes 5/6 and 28 (expressions are chosen by option data, the `faceId`). Outside: AMM plays poses through a workspot entity (`PlayInDeviceSimple` + `SendJumpToAnimEnt`) and NPC expressions through `AnimFeature_FacialReaction`; see [facial expressions](facial-expressions.md) | [runtime] [source] AMM `Modules/anims.lua:344-407`, `Modules/tools.lua:2368-2395` |
...
3. Which `m_editMode` does the in-world mirror pass, and does the native `ReFinalizeState` save or autosave?
...
[Runtime access](runtime-access.md) · [Facial expressions](facial-expressions.md) · [Character-creator lighting](creator-lighting.md) · [Game crashes](game-crashes.md) · [Bridge autonomy backlog](../research/backlog/bridge-autonomy.md) · [Runtime bridge design](../research/runtime/runtime-bridge-design.md) · [Test card](../research/runtime/runtime-bridge-test-card.md)
