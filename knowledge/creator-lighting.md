...
| Surround | Black background, `scene.environment` null, studio key, fill and rim hidden. | None |
...
**The Studio stage beside it.** The ordinary authoring preset is not a model of any game rig; it exists to make shape and shine readable while authoring. `src/studio-lighting.ts` holds its adjustable rig as data and `src/studio-light-rig.ts` applies it to Three: the room environment's strength (`scene.environmentIntensity`, or the light probe's intensity without half float), a key (azimuth `lightAngle`, an elevation, strength × 2.5), the original fill (strength × 1) and a rim from behind V's right (strength × 2.5, off by default), an untinted option that keeps each light's luminance, and ACES exposure from 0.125 to 8 (±3 stops). **Soft studio** is the original rig exactly and the default: the room carries more light than the key (about 1.05 × albedo of ambient against 0.8 × albedo × cos θ from the key), which is why the key's angle used to read only as faint shading. The other setups trade the room for direct light: **Key light** (room 20 %, key 240 % from 305° and 35° up), **Flat** (room 140 %, a faint frontal key, untinted), **Rim / dramatic** (room 8 %, key 280 % from 285°, rim 250 %) and **Mirror** (room 50 %, key 160 % from the front and 20° up, untinted), each with exposure raised to keep skin near the same brightness. Every lit material follows these lights through Three's own light and environment uniforms, including the skin light and the authored plate's single lit pass; `tests/webgl-studio-lighting.test.ts` checks on a real GPU that the key's azimuth and elevation move a Metallic plate's highlight (standard and skin light) and a metal sphere's, that key strength scales the direct light linearly, and that environment strength scales the ambient term with the prefiltered room and with its light probe. The rig is stored in the workspace only when it differs from Soft studio. It is an authoring aid, not evidence about the game's lighting.
