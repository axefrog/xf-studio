...
**Calibration helper.** `bun tools/calibrate-creator-capture.ts --game <png> --studio <png> --patches <json> [--lut <bin>] [--k <number>] [--repeat <png>]` (in `projects/xf-studio/authoring`) samples the §8 patches from a screenshot and a Studio render at the matched camera, fits `k` on the forehead through the LUT's grey response, and prints every pass mark of §8 that the patches allow. Hue and skin ΔE are compared after the fit. It is read-only and writes nothing; `--lut` takes the decoded cube from the private preview cache (`grading-lut/files/`). Its arithmetic is in `src/creator-calibration.ts`, tested on synthetic images.
