...
- **The plate's texture density is the first bottleneck, and the fix is cheap.** The plate uses the head's UVs, so a 1024 atlas gives about 0.4–0.56 mm per texel on the lids. Vanilla eye makeup has its own UV layout that is about 7 times denser. Today's "fine" Shimmer facets are therefore about 1.4 mm wide. `mesh_decal` has UV scale and offset constants, so a plate-local window in a non-square texture spends every texel on the plate. At 2048×512 it has today's texel count and about 0.12 mm per texel; at 4096×1024 it reaches about 0.06 mm [source] [resource].
...
| UV window | Mesh UV U 0.2686–0.7312, V 0.6733–0.8243 (the plate bounds plus a margin, so wrap addressing never samples across an edge). `UVScaleX` 2.161695, `UVOffsetX` 0.000216, `UVScaleY` 6.622517, `UVOffsetY` −1.647682. The V sign must follow the compiler's existing image-row convention; verify it offline on a decoded export before any game test. | [source] formula; sign [hypothesis] |
...
- **The glitter board** ([experiment 018](../experiments/018-glitter-route/README.md#diagnostic-board)): six presets that separate the base recipe, nested against BOX mips, flake width, flake surface, tilt spread and the emissive accent, with a DLSS-against-DLAA check.
