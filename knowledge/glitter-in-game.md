...
| UV window | The production window Build derives from the plate: its stored UV0 bounds widened by 1/64 of the span per side (stored U 0.2659–0.7339, V 0.6739–0.8236 for the built-in plate). `UVScaleX` 2.136780, `UVOffsetX` 0.000261, `UVScaleY` 6.680135, `UVOffsetY` −1.661879. The negative V offset follows from WolvenKit's bottom-to-top row storage and is checked on every build ([experiment 019](../experiments/019-uv-window/README.md)). A Glitter route would use the same constants at 4096×1024. | [source] formula; sign [offline]; placement in game [hypothesis] |
