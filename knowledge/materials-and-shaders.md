...
**Plate texel density.** The plate keeps the head's UV0, so it samples only a 0.453 × 0.145 rectangle (4 %) of each preset texture. At the packaged 1024 size a texel is about **0.56 × 0.40 mm** on the lids (569 and 405 mm per UV unit, area-weighted medians). The vanilla eye-makeup mesh has its own UV layout at about 66 mm per UV unit, so its 1024 texels are 0.065 mm [resource] ([experiment 018](../experiments/018-glitter-route/README.md#texel-density)). This bounds every finish's edge and detail resolution. It is why the Shimmer bake's "fine" facets are about 1.4 mm wide. `mesh_decal`'s `UVScale`/`UVOffset` can map just the plate's rectangle onto a non-square texture instead (4096×1024 gives about 0.06 mm) [source]; the Glitter proposal uses this.
